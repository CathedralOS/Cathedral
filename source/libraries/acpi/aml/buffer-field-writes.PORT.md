...
Status: tested at the bounded byte-write boundary. Final repository-path checks
passed 189 full-store behavior/control pairs, 3 representative constant pairs
and 89 actual public method observations. The new module
...
semantics. This addition is separate from runtime target handling.
...
directory's history/prepublication path with the original manifest. Final
receipts bind the current 12-file imported source/build closure, exact generated
fixture bodies, compiler/runner identities and public Rust probe.

Reproduce from the repository root:

```sh
python3 tools/ports/acpi/aml/buffer-field-writes/fixtures.py --check
python3 tools/ports/acpi/aml/buffer-field-writes/inventory.py --check
python3 tools/ports/acpi/aml/buffer-field-writes/reference.py --write
python3 tools/ports/acpi/aml/buffer-field-writes/check.py --record tools/ports/acpi/aml/buffer-field-writes/verification.json
python3 tools/ports/acpi/aml/buffer-field-writes/check_const.py
python3 tools/ports/acpi/aml/buffer-field-writes/verify_record.py
```
