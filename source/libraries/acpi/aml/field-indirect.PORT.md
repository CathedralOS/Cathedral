...
normal Field milestone at Git revision `24aed04`, with its tested source retained
at `e74cc3e`; it does not complete ACPI-004
...
Canonical integration retains the tested source ancestor. The
[input reconciliation](../../../../tools/ports/acpi/aml/field-namespace/indirect-integration-reconciliation.json)
verifies each isolated receipt's inputs against that Git source and lists later
changes and additional package inputs. The StoreNamed target path is preserved;
its FieldUnit pattern changes only to the canonical binding payload.

A fresh canonical run of the unchanged six boundary pairs passes after merging
with named Store, ObjectType and ToInteger (210.575 seconds). The
[canonical receipt](../../../../tools/ports/acpi/aml/field-bindings/integrated-verification.json)
passes exact current-input verification. The 100 loader and three comparator
pairs remain source-bound to `e74cc3e`; they were not rerun on the combined tree.
