...
Published pure helper; not imported by any executor and not opcode completion.
...
The historical scratch validation (75 behavior/control pairs,3 constant pairs,
47 public observations) is retained under tools/.../byte-literals/history/scratch
with its original hashes. Final publication copies the helper unchanged and
rechecks against current canonical names/namespace guards at Cathedral7629bb8.
Final receipts are separate and bind the exact14-file transitive Omega source
and package-build closure; unrelated ACPI modules are not claimed as tested.

Final publication validation PASS:75 actual Omega behavior/control pairs,3
constant-expression pairs,47 actual public observations (32 same-value
agreements,5 caught panics,4 returned errors). Current dependency hashes and
original historical artifact hashes pass verify_record.py. These are pure
helper results; executor dispatch remains unchanged.
