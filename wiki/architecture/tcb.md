...
> the system's invariants to hold?" It is derived from the design chapters.
> This document owns the enumeration, so a component's trust status is
> determined here and mirrored in its manifest, not inferred from its position in the
> [source tree](repository_layout.md).
>
> The goal is not zero trust, which is impossible, but a small, enumerable,
> stated trusted set. Everything not on this list is
...
system-wide invariant: memory safety, authority confinement, or the integrity
of the capability graph. Trust is a property of what a component can do to the
whole system, not of how important it is. A file browser is critical to daily
use and holds broad capabilities, yet is not trusted. It can only misuse what
it was granted, and revocation contains it. The compositor is trusted, not
...
memory isolation outside shared authority, forcible termination, fault
containment, and bounded resource use. A mechanism receives only the
...
- **The specifications.** The soft, load-bearing entry. A proof shows
  code-meets-spec, never spec-meets-intent; that gap is unformalizable. The
  specifications are kept small, legible, and tested, and "proven" is never
  sold as "correct."
...
- Boundary-provider registry (only registered providers may mint host/hardware
  boundaries; it is the anti-escape-hatch).
...
`source/contracts/` holds the capability vocabulary, syscall surface, wire
schemas, boot handoff, and manifest format. These are not executing code, but
they are the agreed definitions everything targets. A change here redefines
the platform, so the contract layer is trusted the way a constitution is: it
...
| Network broker / demux | Mints flow capabilities; owns the single NIC path. The QUIC classifier loads into its router domain, so `RegisterClassifier` is privileged. |
...
## Not trusted (contained-not-trusted)

Everything else is confined by construction:

- **All drivers.** IOMMU-confined; a compromised driver kills its device, not
  the machine.
- **All applications.** Capability-confined; they hold only granted, revocable
...
- **All checked Omega libraries.** Ordinary linked code with no ambient
  authority. Opaque native libraries are rejected or isolated; an admitted
  uncontained one joins the containing scope's TCB and marks its manifest
  incomplete.
...
- **Telemetry collectors, foreign/legacy code, the compatibility box.** Walled;
  foreign binaries run behind the hardware wall with no proof guarantee.

The line is not intra-vs-cross-app; it is proven-connected-graph vs unproven
boundary. Proof, and therefore trust that can be earned rather than assumed,
stops at the first unproven or foreign node.

---

## Keeping this list accurate

- This document is the one place the trusted set is enumerated, and every
