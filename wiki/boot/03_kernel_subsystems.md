...
- **Memory manager.** The final firmware map mints physical extents; page-table operations consume frame authority and establish installable tables; mappings mint attenuated virtual/device extents; Arenas allocate only from appropriate backing extents. Executable publication, TLB shootdown, and IOMMU invalidation are explicit provider transitions ([hardware foundation](../design/part_0_foundations/03_hardware_foundation_profile.md), [memory & persistence](../design/part_2_components/02_memory_and_persistence.md)).
