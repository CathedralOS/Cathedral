...
the requested table slot and covers the policy-owned subset before placement:
exact vector, IST, fatal disposition, present/ring-0 interrupt-gate attributes,
and reserved-zero. Its `PolicyConsistent` result is ordinary candidate data,
not a materialization receipt. Exact handler identity and code selector
validation remain at the admitted resolver/source-carrier and boot-selected
segment seam.
