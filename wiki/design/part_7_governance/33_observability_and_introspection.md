...
The key idea is **system-wide causality**: Cathedral records a **causal event graph** from the first boot. An event is not a log line; it is a node with edges to the events that caused it and the authority that permitted it. Observability is then just *querying that graph*. The same graph feeds audit ([[34_audit_compliance_provenance]]).
...
- **Causal vs. wall-clock ordering.** The graph is causal first; reconciling it with timestamps across components needs trusted time ([[12_time_and_clocks]]).
...
- **Privacy of observation.** "What app read this record" is itself sensitive; introspection must not become a side channel ([[08_data_model_and_privacy]]).
...
- Is the authority graph materialized continuously, or reconstructed on demand from the event graph (the open question raised in [[03_capability_model]])?
