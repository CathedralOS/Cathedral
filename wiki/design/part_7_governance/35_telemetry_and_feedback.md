...
- **Crash report hygiene.** Crash payloads must be scrubbed of secrets and capability material by construction, not by best-effort redaction ([[07_secrets_and_keys]]).
...
- **Organization control.** An organization or tenant sets telemetry policy as enforced, attested policy, not a checkbox ([[31_multi_user_and_org_control]]).
...
- **Attenuation / leasing** make telemetry grants narrow and expiring by default ([[04_capability_lifecycle]]).
