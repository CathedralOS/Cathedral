...
- **Interrupt stack/nesting policy.** Pick Cathedral's initial x86 stack classes, masking/preemption graph, and WCSU composition, then derive the concrete IST fields from that one normalized policy. Omega's provider-neutral installed-root ledger and deterministic `external_roots.json` manifest are now live and retain the chosen `BoundaryEntryPlan`, exact installation binding, nesting relation, WCSU demand, effects/receipts, and component pins; Cathedral still owns the concrete policy and provider wiring ([[hardware_foundation_profile]]).
