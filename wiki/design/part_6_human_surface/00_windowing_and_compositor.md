...
## Swappable shell and chrome

The shell, the panel, the desktop, and the lock screen are components filling slots the session binds, shipped through the same store as any app. Swapping one is installing the component and rebinding the slot, and because slots are bound at the session level they can be re-pointed live, the way the world chooser flips a sandbox under a running app ([[human_permission_ux]]). A motivated user can replace the entire visible OS, up to running a game as the shell that spawns the file explorer through its launch capability and composites it into its own scene.

This is safe only because the security model never lived in the chrome. Two things stay OS-owned and are not slots. The capability core, so a custom shell is an ordinary app holding shell-tier capabilities and nothing more. And the trusted-path layer, so login, unlock, and grant prompts are OS-drawn, always on top, and always reachable, and a custom shell cannot forge them. A real prompt appears as an OS overlay even over a fullscreen custom shell, and a session key combo the shell cannot intercept summons the OS surface. Recovery is required: a faulted shell is restarted by the supervisor, a broken one falls back to a default, and reverting is repointing the slot at the previous content-addressed version.
...
- What stays OS-owned and non-swappable (the trusted-path layer, the capability core) versus what is a rebindable shell/chrome/desktop slot, and how is recovery to a default shell guaranteed?
