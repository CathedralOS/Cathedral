...
**Agents get a ceiling'd lease — which is a virtual card.** An autonomous agent cannot tap-confirm each charge, so the human mints *once* a leased, ceiling'd action capability ("≤ $50 per charge, ≤ $200 this session, this merchant") — mechanically identical to a merchant-locked, amount-capped, revocable **virtual card** ([[agents_as_principals]]). The capability's dependent bounds are the spend limit; service reach only says which payment boundary may be reached.
