...
- **Delegation is a syscall.** Moving authority between principals inserts an entry into the receiver's table — checked against the receiver's manifest ceiling *at insert*, so a table only ever contains manifest-compatible grants and redemption never re-checks the manifest. Every insert records the parent edge, which means the arena *is* the materialized authority graph and provenance is recorded by construction ([[capability_model]]).
...
- The runtime representation is settled (the arena); what is the exact entry schema, and the minimal kernel surface for insert/attenuate/revoke?
