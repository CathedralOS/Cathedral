...
- **Effect and boundary ceilings.** Beyond *which* objects, policy bounds *what kind* of behavior (`filesystem_io`, `network_io`, `device_io`) and *which provider* may implement it ([[24_driver_model]]).
- **Secrets, clipboard, capture, media devices.** Each is a capability surface, not a special case — clipboard read, screenshot, audio, and camera are grants with the same lifecycle ([[07_secrets_and_keys]], [[04_capability_lifecycle]]).
- **Inter-app communication.** IPC is capability transfer; the policy that bounds what an app may *do* must also bound what it may *hand to a peer* ([[15_ipc_and_service_invocation]]).
- **Policy introspection & revocation UX.** A policy that cannot be read back, or a revocation a human cannot understand the consequences of, is not a policy ([[28_human_permission_ux]], [[33_observability_and_introspection]]).
- **Auditing.** Every grant, denial, and combination check is an event in the provenance record ([[34_audit_compliance_provenance]]).
...
- A policy is a set of **ceilings over inferred authority flow** — the accepts/uses/derives/stores report from [[03_capability_model]] is exactly the surface a ceiling bounds.
...
- Are combination rules global, per-publisher, per-org, or per-data-class ([[08_data_model_and_privacy]])? Who is authoritative when they conflict?
- Can "no dangerous combination is reachable" ever be proven statically, or is it inherently a runtime graph query?
- How are policy ceilings versioned and migrated when an app updates and its authority footprint changes ([[23_updates_and_hot_swap]])?
