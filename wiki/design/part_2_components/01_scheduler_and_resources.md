...
- **Cooperative vs preemptive, reconciled (the keystone).** This chapter assumes *both* run-to-completion parking — which the Omega model supports cleanly, since a parked task is data, not a stack — *and* preemptive context switches (priority inheritance, deadline scheduling, the per-switch side-channel flush). But the Omega concurrency model is stackless and parks *only at `await`*: it explicitly **cannot represent a PREEMPTED task's full register/stack state at an arbitrary instruction boundary** ("a transition does not push a frame"; suspend-in-call is forbidden). So Cathedral must define the context-switch / save-restore representation for a preempted task, and likely write the kernel scheduler itself in a **restricted static-task subset** (static tasks, raw atomics + MMIO, explicit context switch, no `spawn`/`await`) to break the bootstrap regress — the scheduler cannot be written in terms of an `await` model that lowers *through* the scheduler. (Oxide's Hubris ships exactly this static-task shape.) This is the single most important unresolved piece of the bare-metal story — see Omega [concurrency_atomics.md 2026-06-15 review](../../../../Omega/wiki/design_briefs/concurrency_atomics.md) (OQ2).
