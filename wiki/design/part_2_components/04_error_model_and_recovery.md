...
**Taxonomy: closed dispositions + open causes, protocol-encoded.** The
**recovery disposition is the closed, frozen set** (`retry | restart | migrate
| escalate | fatal`); the extensible **`FailureCause`** uses a numbered
protocol schema with an explicit unknown-cause policy. A new OS can therefore
emit detail an old app does not know while the old app still reacts to the
closed disposition. The closed set also carries outcome certainty: a timed-out
call is `Unknown` (maybe happened; reconcile by idempotency key), distinct from
a clean `Rejected` (definitely not done and safe to retry).
...
- **Taxonomy — decided:** closed recovery dispositions plus an extensible
  numbered `FailureCause` protocol schema, so old apps retain the disposition
  even for unknown detail.
