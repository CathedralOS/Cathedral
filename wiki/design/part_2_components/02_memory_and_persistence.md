...
- **Versioned persistent data.** Format packages define immutable era schemas as ordinary data, exhaustive lineage sums, codecs, and migration machines. Omega supplies the identity, layout, proof, and admission hooks; it does not provide a magical `Versioned<T>` container ([[versioned_state_and_migration]]).
