...
Honestly defensible, kept modest: the OS is *already* the persistence layer, so
this is an API-shape question on an existing subsystem rather than a new reason
to exist. The real win is fewer internal seams and less proof surface. The
genuinely OS-level properties are cross-app coherence and unforgeable authority
surviving a crash. The combination that makes the model survivable is the
borrow checker, capabilities as values, crash-atomic transitions, and typed
migration over explicit historical schemas at the codec staleness boundary.
