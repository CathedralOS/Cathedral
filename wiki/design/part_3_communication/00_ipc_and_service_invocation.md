...
- **The mailbox is `many_to_one`, and that *is* the actor.** A single consumer processing one message at a time is what lets its `self` state mutate lock-free — only one task ever touches it. This is the blessed concurrency shape: producers post variants of one sum, the consumer does one wait + one transition (the no-select model, [Omega ch18](https://github.com/CathedralOS/Omega/blob/main/wiki/language_guide/chapter_18_concurrency.md)). Request/reply either rides the shared mailbox with a correlation token (to discard stale/late replies) or uses a dedicated **oneshot**, where the channel's identity *is* the correlation — ring-free and token-free, at the cost of a channel per outstanding request.
