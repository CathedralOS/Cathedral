...
- [ ] `time_and_clocks` — *much decided* (see chapter "The decided mechanism"): clock-as-capability is zero-per-read (the cap picks the page once; read = cheap userspace counter); virtual dilation has two tiers — cooperative software scale/offset (fast, default, *bypassable* and that's fine since everything's a Matrix) vs non-bypassable counter-gate (`CR4.TSD`/`CNTKCTL` trap, slow, rare); **no VT-x for native Matrices** (reserved for the compat box); obfuscating the scale is futile (adversary measures the rate); `Clock::Trusted` = anti-rollback monotonic counter + Roughtime + measured-boot (RTC = cold-start baseline only); ARM is 1:1; a **custom capability/time-native ISA dissolves the cost/leak/bypass trade-offs by construction**. **Residual: virtual-time composition (what time a real-clock service sees when a virtual-clock component calls it).**
