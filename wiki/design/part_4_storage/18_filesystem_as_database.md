...
Cross-realm sharing is a capability-gated edge, not a global path. When an app needs a user file, something mints a capability into the user realm for that one object: that is the file picker as an authority mint ([[28_human_permission_ux]]). The distinction to keep explicit, which iOS and Android both blur, is app-private storage (the app realm) versus user-facing documents (the user realm): a file an app saves "for the user" lands in the user realm via a granted capability, not buried in the app's container.
...
- **Records and views.** Files as typed records; directories as indexes/views; a query layer ([[33_observability_and_introspection]]) over metadata rather than `find`/`stat` loops. Search/indexing is a first-class service, scoped to the realms you hold.
- **The transaction log as the source of truth.** Journaling is exposed *as an API*: change data capture is reading the log forward from an offset. Snapshots are log positions; rollback is replay to a position ([[19_transactions_and_consistency]]). The log is likely per-realm.
...
- **Atomic multi-object change.** A rename, a config write, and a content update commit as one transaction; the primitive lives in [[19_transactions_and_consistency]] and this chapter is its first heavy consumer.
...
- **The system realm is swappable.** It wants to be immutable and content-addressed so the OS can version and roll it back independently of the user realm ([[21_versioned_state_and_migration]], [[23_updates_and_hot_swap]]); reinstall swaps the system realm and leaves the user realm untouched.
- **Per-object capabilities.** A handle to an object is the authority over it; a directory view is an attenuated capability ([[03_capability_model]]), and a realm root is just the top-most such capability.
- **Provenance.** Each record carries who/what/when wrote it ([[34_audit_compliance_provenance]]).
- **Schema evolution.** Records have typed, versioned shapes that migrate ([[21_versioned_state_and_migration]]).
...
- What is the right realm granularity: one realm per app, or coarser, and who is allowed to mint a realm before it becomes fragmentation ([[37_governance_and_extension_boundaries]])?
...
- How are durable subscriptions garbage-collected when a subscriber dies without unsubscribing: lease-based cursors ([[04_capability_lifecycle]])?
...
## Related
- [[03_capability_model]] — per-object capabilities, realm roots, and attenuated directory views.
- [[30_naming_and_discovery]] — realm-qualified names and resolution as a security primitive.
- [[28_human_permission_ux]] — the file picker as the cross-realm capability mint.
- [[31_multi_user_and_org_control]] — tenants as realms.
- [[19_transactions_and_consistency]] — the atomic-commit primitive this chapter consumes.
- [[21_versioned_state_and_migration]] — schema evolution and the swappable system realm.
