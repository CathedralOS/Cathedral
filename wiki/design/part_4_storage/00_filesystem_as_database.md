...
- The `hash -> location` index is the mandatory analog of ZFS's dedup table, because content-addressing makes dedup the identity rather than an optional feature you can turn off. Can it be tiered cheaply (hot entries in RAM, the rest on SSD), or does full content-addressing impose a real memory floor at large scale?
