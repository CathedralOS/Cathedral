...
Three honest limits. Porting a legacy database wholesale still works but is wasteful: it runs as an opaque blob with its own redundant log and cache, gaining nothing the OS already provides. The store is general-purpose, so a specialized engine (a particular locking or isolation model, vector or columnar indexes, time-series layouts) still builds its own structures, now over a durable, integrity-checked, snapshotted object backing instead of raw blocks. And "the runtime owns placement" gives up some of the low-level control a tuned engine wants, with only the placement and advice hooks giving it back. The embedded-database case (SQLite for app state) mostly disappears, because app state is just objects in the store.
