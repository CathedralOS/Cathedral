...
- **Build-time evaluation + the complete normalized contract** make config-as-a-machine safe: the concrete `configure` invocation must fit the evaluator's floor (empty service reach, total, no unresolved failure/trap/abort route, and no blocking, suspension, or ambient authority). The reach row is necessary but not sufficient. The result is typed data rather than an opaque script — see Omega [build & package model](https://github.com/CathedralOS/Omega/blob/main/wiki/design_briefs/build_and_package_model.md) and [build-time evaluation](https://github.com/CathedralOS/Omega/blob/main/wiki/design_briefs/build_time_evaluation.md).
