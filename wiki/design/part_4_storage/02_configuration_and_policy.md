...
- **Policy layering.** Precedence across user / organization / device / app layers, with a defined, inspectable resolution rule ([[multi_user_and_org_control]]). Organization policy can *pin* or *cap* a value the user may otherwise set.
- **Temporary overrides.** Scoped, expiring overrides (a debug flag for one session) modeled as leases ([[capability_lifecycle]]), not permanent edits.
- **Auditable changes, diff, rollback.** Every change records who/what/when and is reversible — the config store is itself a small versioned object graph ([[filesystem_as_database]]).
...
- How do unmanaged legacy apps ([[filesystem_as_database]] compatibility) read config they expect as text files, projected from the typed store?
...
- [[multi_user_and_org_control]] — the policy layers and who owns each.
- [[secrets_and_keys]] — secret-valued settings kept out of the config store.
