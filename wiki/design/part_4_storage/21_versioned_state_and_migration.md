...
Historical shapes coexist as **named versions** of a typed `data`, and a migration is **typed code** — a migration machine — that transforms an old shape into the current one under explicit effect, ownership, and invariant obligations. Migrations compose along a chain (`v1 -> v2 -> current`), so state from any known era can be carried forward by stitching known steps. Crucially, this chapter owns *shape continuity only*. The **operational** act of updating a running system — reaching quiescence, draining work, rolling upgrades, dependency planning, and operational rollback — belongs to [[23_updates_and_hot_swap]]. We define the typed transform; that chapter decides when it runs and on whose schedule.
...
- **Migration fallibility & rollback.** Whether a migration is infallible or may fail — and if it can fail, what the abort/rollback story is — must be explicit, feeding the transactional model ([[19_transactions_and_consistency]]).
- **Version matching at boundaries.** Places that can legitimately hold mixed versions: hot-swap state stores ([[23_updates_and_hot_swap]]), saved snapshots ([[18_filesystem_as_database]]), replicated state, and persisted memory ([[11_memory_and_persistence]]). Normal current-version code should not pay a version tag everywhere.
...
- **Where migrated state lives.** Filesystem records, config, capabilities, and IPC protocol versions ([[15_ipc_and_service_invocation]]) are all migration participants sharing this one primitive.
...
- **Ownership / borrowing** supplies the `exclusive(old)` / frozen-access facts a migration needs; the swap-time obligations themselves are owned by [[23_updates_and_hot_swap]].
...
- How do migrations interact with content-addressed bodies that many records share ([[18_filesystem_as_database]]) — migrate the reference or the content?
