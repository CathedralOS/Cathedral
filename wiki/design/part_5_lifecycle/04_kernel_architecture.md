...
- Miscompilation is now answered by the TCB-minimization strategy above (bootstrap seed + small checker + verified translation); the residue *inside the proved core* is **side channels** the type system does not model, plus the irreducible {seed, checker, specs, hardware} base — small and human-audited, never zero. (Drivers are excluded — hardware-confined, not language-trusted, [[driver_model]].)
