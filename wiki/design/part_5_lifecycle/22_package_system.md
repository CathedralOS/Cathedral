...
- **Capability manifest.** What the package `accepts`, `uses`, `stores`, `derives`, and `acquires` (from [[03_capability_model]]) is the heart of the manifest. No ambient filesystem, network, or device access during install.
- **State schemas & migration.** A package owns persistent state shapes; upgrades ship versioned data + migration machines (see [[21_versioned_state_and_migration]]).
- **Atomic install / uninstall.** Commit or refuse as one transaction ([[19_transactions_and_consistency]]); no half-installed state, no orphaned hooks.
- **Reproducible, hermetic builds + signed provenance.** The same source yields the same artifact; the build's inputs and signer are recorded ([[34_audit_compliance_provenance]]).
...
- **Revocation & staged rollout.** Package-level revocation, staged/canary rollout, and rollback are first-class, gated by the store/control plane ([[36_store_and_economic_control]]).
...
- **`wire data`** ([[15_ipc_and_service_invocation]]) declares the protocols spoken, making protocol compatibility part of the manifest.
...
- How are native/foreign components (a legacy box, see [[41_compatibility_and_legacy]]) packaged without contaminating the proof-carrying model?
