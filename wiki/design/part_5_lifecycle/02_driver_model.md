...
- **Interrupt = message.** The only kernel-mode code on the path is a tiny,
  device-agnostic, fixed-work IRQ root: it acknowledges/masks the line and
  turns the interrupt into a wakeup for the parked driver actor (the scheduler's
  existing park/unpark, [[scheduler_and_resources]]). Level state such as the
  timer uses a preallocated coalescing wake; information-bearing edges use a
  bounded ring with an explicit overflow policy. No device-specific fan-out
  runs privileged. Extreme-throughput devices skip interrupts and **poll** —
  the driver busy-reads a DMA ring with no kernel hop, the shared-region
  primitive ([[ipc_and_service_invocation]]) as the ring.
...
- **DMA reach is per transfer, not ambient.** Omega now refuses an external
  ...
  subrange transfer, so activation stacks and other storage outside that
...
- **Evolving driver APIs.** Kernel↔driver and driver↔client interfaces use ordinary numbered schemas, layout/codec policies, immutable historical shapes, and checked conversions. Live driver replacement is a separate component protocol; neither requires `wire data` nor `Versioned<T>` syntax.
