...
- **Browser-first / web app model.** Leans on the web as the runtime ([[32_web_integration]]); large existing catalog, capability-shaped surface.
...
- **Effects + boundary** make the compatibility box a `boundary` provider with a hard effect ceiling — its blast radius is a compiler-checked fact, not a hope.
...
- Does the web app model ([[32_web_integration]]) make a native Linux personality unnecessary, letting Cathedral refuse it outright?
