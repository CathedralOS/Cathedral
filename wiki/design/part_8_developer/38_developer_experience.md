...
- **SDK & package tooling.** One toolchain that builds components, resolves versioned dependencies, and publishes to a local store ([[22_package_system]], [[36_store_and_economic_control]]) — not N disconnected tools.
...
- **Capability visualizer.** Render the per-component authority-flow report as a graph: accepts / uses / derives / **stores** / acquires / returns / releases. Stored authority is highlighted because it is the dangerous, long-lived case ([[03_capability_model]]).
- **Protocol explorer.** Diff `wire data` schemas across versions and *name the breaking change* before publish — the field that moved, the compatibility rule that broke ([[15_ipc_and_service_invocation]]).
- **Migration tester.** Prove a versioned-state migration is total over the prior shape, and flag the case it does not cover ([[21_versioned_state_and_migration]]).
- **Deadlock / quiescence checker.** Show whether an upgrade can reach quiescence, and which outstanding borrow or wait blocks the swap ([[23_updates_and_hot_swap]]).
- **Debugger, tracer, simulator, deterministic replay.** First-class, shared, and capability-governed — detailed in [[39_debugging_and_tracing]] and [[40_testing_and_simulation]].
- **Resource profiler & certification tool.** Effect ceilings and resource bounds as a pre-certification checklist the developer runs locally before the store gate ([[36_store_and_economic_control]]).
