...
**Only small inductive certificates *transmit*; exhaustion stays local.** A proof (a small inductive certificate) ships in the content-addressed closure and a consumer **re-checks it locally and cheaply** — the one regime where trust-by-checking genuinely holds. **Exhaustion does not transmit**: brute-force has no certificate (pure attestation), and a large certificate collapses into trusting a checking-*server* (attestation-with-extra-steps, [Omega proof_caching](../../../../Omega/wiki/design_briefs/proof_caching.md)). So **Cathedral does not do proof-by-exhaustion as a *shipped guarantee*** — TLC-enumeration and randomized sampling are *local* bug-finders/confidence tools for the developer, not transmissible certificates a consumer can cheaply check. The escape hatch is the **budget/bound measure** ([Omega totality](../../../../Omega/wiki/design_briefs/totality_and_bounded_computation.md)): re-express a would-be-exhaustion as a bounded computation (the bound *is* the measure → cheap, small cert, ships) — but get greedy with the bound and your build times suffer.
