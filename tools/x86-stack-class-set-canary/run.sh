#!/usr/bin/env bash
# Compile and inspect Cathedral's complete bootstrap stack-class policy without
# provisioning storage, materializing a TSS/IDT, or installing any root.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
CANARY_ROOT="$REPO_ROOT/tools/x86-stack-class-set-canary"
OMEGA_REPO="${OMEGA_REPO:-$REPO_ROOT/../Omega}"
SCRATCH_DIR="$(mktemp -d "${TMPDIR:-/tmp}/cathedral-x86-stack-class-set.XXXXXX")"
...
if ! command -v jq >/dev/null 2>&1; then
  echo "error: jq is required to validate x86 stack-class artifacts" >&2
  exit 2
fi
...
ASSERTIONS="$CANARY_ROOT/assert-set.jq"
...
echo "Cathedral x86 stack-class-set canary passed"
