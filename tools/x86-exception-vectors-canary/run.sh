#!/usr/bin/env bash
# Compile Cathedral's pure exception-vector snapshot without establishing an
# interrupt entry or exercising privileged hardware.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
CANARY_ROOT="$REPO_ROOT/tools/x86-exception-vectors-canary"
OMEGA_REPO="${OMEGA_REPO:-$REPO_ROOT/../Omega}"
SCRATCH_DIR="$(mktemp -d "${TMPDIR:-/tmp}/cathedral-x86-exception-vectors.XXXXXX")"
...
if ! command -v jq >/dev/null 2>&1; then
  echo "error: jq is required to validate the typed exception-vector snapshot" >&2
  exit 2
fi
...
VECTOR_ASSERTIONS="$CANARY_ROOT/assert-vectors.jq"

for artifact in "$TYPED" "$CONTRACTS" "$VECTOR_ASSERTIONS"; do
  [[ -f "$artifact" ]] || {
...
jq -s -e -f "$VECTOR_ASSERTIONS" "$TYPED" "$CONTRACTS" >/dev/null

echo "Cathedral x86 exception-vector fact-table canary passed"
