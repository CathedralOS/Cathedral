#!/usr/bin/env bash
# Compile and jointly inspect Cathedral's pre-installation exception/timer
# bootstrap pieces without invoking hardware or claiming installation.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
CANARY_ROOT="$REPO_ROOT/tools/x86-legacy-bootstrap-composition-canary"
OMEGA_REPO="${OMEGA_REPO:-$REPO_ROOT/../Omega}"
SCRATCH_DIR="$(mktemp -d "${TMPDIR:-/tmp}/cathedral-x86-legacy-bootstrap.XXXXXX")"
...
if ! command -v jq >/dev/null 2>&1; then
  echo "error: jq is required to validate bootstrap-composition artifacts" >&2
  exit 2
fi
...
COMPOSITION_ASSERTIONS="$CANARY_ROOT/assert-composition.jq"

for artifact in "$TYPED" "$CONTRACTS" "$COMPOSITION_ASSERTIONS"; do
  [[ -f "$artifact" ]] || {
...
jq -s -e -f "$COMPOSITION_ASSERTIONS" "$TYPED" "$CONTRACTS" >/dev/null

echo "Cathedral x86 legacy bootstrap-composition canary passed"
