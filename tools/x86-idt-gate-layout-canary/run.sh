#!/usr/bin/env bash
# Compile Cathedral's pure x86 IDT gate geometry without resolving an entry,
# publishing a table, or granting any privileged operation.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
CANARY_ROOT="$REPO_ROOT/tools/x86-idt-gate-layout-canary"
OMEGA_REPO="${OMEGA_REPO:-$REPO_ROOT/../Omega}"
SCRATCH_DIR="$(mktemp -d "${TMPDIR:-/tmp}/cathedral-x86-idt-gate.XXXXXX")"
...
if ! command -v jq >/dev/null 2>&1; then
  echo "error: jq is required to validate the typed x86 IDT gate layout" >&2
  exit 2
fi
...
echo "Cathedral x86 IDT gate layout canary passed"
