#!/usr/bin/env bash
# Compile and inspect Cathedral's checked PIC initialization leaves without
# invoking them or accessing privileged hardware.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
CANARY_ROOT="$REPO_ROOT/tools/pic-8259-initialization-contract-canary"
OMEGA_REPO="${OMEGA_REPO:-$REPO_ROOT/../Omega}"
SCRATCH_DIR="$(mktemp -d "${TMPDIR:-/tmp}/cathedral-pic-8259-init.XXXXXX")"
...
if ! command -v jq >/dev/null 2>&1; then
  echo "error: jq is required to validate PIC initialization artifacts" >&2
  exit 2
fi
...
PROVIDER_ASSERTIONS="$CANARY_ROOT/assert-initialization.jq"
...
echo "Cathedral 8259 PIC initialization-contract canary passed"
