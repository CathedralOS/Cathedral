...
path=ROOT/'source/contracts/uefi/raw/console.vectors.json'
...
for key,expression in re.findall(r'println!\("([^"=]+)=\{\}", (.+)\);',source):
...
for expression,key in re.findall(r'\{ let value = (.+); let bytes: \[u8; 16\] = value.to_bytes\(\); print!\("([^"=]+)="\)',source):
