...
SOURCES=['source/libraries/x86_64/'+n for n in ['build.omg','encrypted_mapping_plans.omg','encrypted_mapping_routes.omg','encrypted_recursive_routes.omg','memory_encryption.omg','mapping_plans.omg','mapping_routes.omg','page_entries.omg','pages.omg','addresses.omg']]+['source/drivers/facts/build.omg','source/drivers/facts/x86_page_table_entry.omg']
def snapshot():
...
def main():
 ...
 metadata=json.loads((HERE/'fixtures.json').read_text());before=snapshot();imports=set();bodies=[];selections=[]
 positives=[r['path'] for r in metadata['batches']]+['leaf_checks.omg','extras.omg']
 controls=metadata['controls']+[{'path':'extras.omg','old':'expected==327680','new':'expected==327681','family':'capture-mismatch-payload'},{'path':'extras.omg','old':'result.edits.allocation_calls==1','new':'result.edits.allocation_calls==0','family':'invalid-allocation-attempt-retained'}]
 counts=[0,0]
 for path,change in [(s,None) for s in positives]+[(c['path'],c) for c in controls]:
  name=(Path(path).stem+'_positive')if change is None else change['family'].replace('-','_')+'_control'
 ...
 suite='\n'.join(sorted(imports))+'\n data Suite {}\n'+'\n'.join(bodies)
 with tempfile.TemporaryDirectory(prefix='cathedral-encrypted-mapping-checked-') as temp:
  work=Path(temp);(work/'main.omg').write_text(suite);(work/'checks.omg').write_text((HERE/'checks.omg').read_text());(work/'build.omg').write_text((HERE/'build.omg').read_text().replace('../../../source/',str(ROOT/'source')+'/'))
