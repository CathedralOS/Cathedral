#!/usr/bin/env python3
"""Execute actual bounded geometry/chunk bodies against independent interval vectors."""
...
def snapshot():
 ...
 paths += list((ROOT/'source/libraries/acpi/field_access').glob('*.omg'))
 ...
 paths += [ROOT/'source/libraries/acpi/interpreter'/name for name in ['build.omg','integers.omg']]
...
def build_text():
 text='machine build(builder:&mut Build){builder.application("cathedral-field-access-tests");builder.freestanding=true;'
 for alias,folder in [('access','field_access'),('fields','aml/fields'),('integers','interpreter'),('aml','aml')]:text+='builder.depend_as("'+alias+'",Source::Path {location:"'+str(ROOT/'source/libraries/acpi'/folder)+'"});'
 return text+'}'
...
def main():
 p=argparse.ArgumentParser();p.add_argument('--runner',type=Path,default=RUNNER);p.add_argument('--case',action='append');p.add_argument('--batch-size',type=int,default=40);p.add_argument('--record',default='checked-verification.json');a=p.parse_args()
 ...
 inputs=snapshot();binary=sha(a.runner);start=time.monotonic();batches=[]
 for offset in range(0,len(rows),a.batch_size):
  selected=rows[offset:offset+a.batch_size];text,names=fixtures.render(selected)
  with tempfile.TemporaryDirectory(prefix='cathedral-field-access-checked-')as d:
  ...
  batches.append(dict(cases=[r['name']for r in selected],source_sha256=hashlib.sha256(text.encode()).hexdigest(),output=output))
 record=dict(stage='actual checked interpreter bodies; no native or namespace evaluation',omega_revision=PIN,input_sha256=inputs,runner_sha256=binary,runner_path=str(a.runner),scope='selected'if a.case else'full',cases=[r['name']for r in rows],positive_count=len(rows),control_count=len(rows),elapsed_seconds=round(time.monotonic()-start,3),batches=batches)
