#!/usr/bin/env python3
"""Actual public pinned Interpreter Field accesses into inert initialized memory."""
...
def table(offset,length,flags,region,write=False):
 ...
 body=b'\x70'+aml.integer(0x123456789abcdef0)+b'FLD0\xa4\x00'if write else b'\xa4FLD0'
 return declaration+aml.method('MAIN',body)
def cases():
 rows=[]
 for vector in vectors.cases():
  if vector['patch'] or vector['offset']>4000 or vector['length']>2048:continue
  for write in [False,True]:
   rows.append(dict(name=vector['name']+('_write'if write else'_read'),offset=vector['offset'],length=vector['length'],flags=vector['flags'],region=vector['region'],revision=1 if vector['size']==4 else 2,write=write,strict_expected=vector['expected']))
 return rows
...
def main():
 ...
 target=Path('/tmp/cathedral-field-access-public')
 with tempfile.TemporaryDirectory(prefix='field-access-reference-')as d:
  ...
  for row in cases():
   data=table(row['offset'],row['length'],row['flags'],row['region'],row['write']);file=work/'table.aml';file.write_bytes(data)
 ...
 record=dict(stage='actual public Interpreter load_table/evaluate with inert Vec callbacks; no hardware, private mirror or Omega native execution',pin=PIN,upstream_sha256=source,binary_sha256=sha(binary),source_sha256={p.name:sha(p)for p in [HERE/'reference.py',HERE/'reference.rs',HERE/'aml_encoding.py',HERE/'vectors.py',lock]},rows=rows)
 ...
 print('PASS',len(rows),'public field observations')
