...
def snapshot():
    ...
    paths += [HERE / 'fixtures.py', HERE / 'check.py', fixtures.BASE,
...
def build_source():
    ...
    return ('machine build(builder:&mut Build){\n'
            'builder.application("cathedral-field-mid-integration");builder.freestanding=true;\n'
...
def main():
    ...
    if args.verify:
        ...
        assert record['exit_code']==0 and record['source_unchanged'] and record['omega_revision'] == PIN
        assert digest(Path(record['runner']).read_bytes())==record['runner_sha256']
    ...
    source, entries = render(rows)
    build = build_source()
    with tempfile.TemporaryDirectory(prefix='cathedral-field-mid-integration-') as directory:
        work = Path(directory)
    ...
    if args.record:
        record = dict(format='cathedral-field-read-checked-v1',
                      ...
                      scenario_count=len(rows), control_count=len(rows), exit_code=exit_code,
    ...
    assert exit_code==0, 'Compilation/execution failed; diagnostic retained'
    assert sum(line.startswith('PASS ') for line in lines)==len(entries)
    print(f'PASS {len(rows)} combined interaction pairs; hardware/native not run')
