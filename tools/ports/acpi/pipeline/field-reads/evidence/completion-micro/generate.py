...
out+=copied(pipeline,'complete', [('true -> received(session,value)', 'true -> receive(session,value)'), (' state failed(session:&mut Session,request:NativeRead,outcome:ExecutionOutcome)', ' state receive(session:&mut Session,value:u64)->ExecutionOutcome {let result:ExecutionOutcome=received(session,value);result}\n state failed(session:&mut Session,request:NativeRead,outcome:ExecutionOutcome)')])
out+=copied(pipeline,'received')
...
changes=[('geometry::plan','geometry_plan'),('buffer_fields::field_to_integer','field_to_integer'),('chunks::extract','chunk_extract'),('conversions::integer_to_buffer','integer_to_buffer'),('buffer_fields::copy_bits','copy_bits')]
changed=assembly
...
out+=copied(chunks,'valid')
out+=copied(chunks,'extract',[('pub machine extract(','pub machine chunk_extract(')])
...
machine pending_exact(session:&Session,index:u64)->bool {
 transition index<5 {true -> checked(session,index) _ -> (false)}
 state checked(session:&Session,index:u64)->bool {
...
machine feed(session:&mut Session,index:u64)->bool {
 transition index<5 {true -> checked(session,index) _ -> (false)}
 state checked(session:&mut Session,index:u64)->bool {
