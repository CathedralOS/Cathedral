#!/usr/bin/env python3
"""Check actual AML/provider write composition with exact input evidence."""
...
def snapshot():
    ...
    paths += [HERE/'fixtures.py',HERE/'check.py',HERE/'rejection-helpers.omg',fixtures.COMPARATOR,ROOT/'tools/ports/acpi/interpreter/execution/checked_runner.rs',ROOT/'tools/ports/acpi/interpreter/execution/runner.Cargo.lock']
    return {str(p.relative_to(ROOT)):sha(p) for p in sorted(set(paths))}
def build():
    text='machine build(builder:&mut Build){builder.package("field-write-pipeline");builder.freestanding=true;'
    for alias,path in [('aml','source/libraries/acpi/aml'),('execution','source/libraries/acpi/interpreter/execution'),('integer_helpers','source/libraries/acpi/interpreter'),('pipeline','source/libraries/acpi/pipeline'),('write_values','source/libraries/acpi/field_writes'),('access','source/libraries/acpi/field_access')]:
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='cathedral-write-pipeline-') as name:
