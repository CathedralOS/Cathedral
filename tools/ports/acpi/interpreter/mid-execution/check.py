...
GROUPS = ['execution', 'bridge', 'integer', 'generic', 'pipeline', 'to_integer']
...
def module_source(group, selected):
    ...
    receiver = ''.join(part.title() for part in group.split('_'))+'MidSuite'
...
def main():
    ...
    def run_batch(batch):
        ...
        with tempfile.TemporaryDirectory(prefix='cathedral-mid-execution-') as directory:
            work=Path(directory)
