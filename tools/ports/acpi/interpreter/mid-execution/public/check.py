#!/usr/bin/env python3
"""Pinned public Mid observations using the unchanged inert service-trap harness."""
...
def snapshot():
 paths=[Path(__file__),HERE.parent/'fixtures.py',HERE.parent/'execution-cases.json',HERE.parent/'toolchain.json',PROBE/'src/main.rs',PROBE/'Cargo.toml',PROBE/'Cargo.lock',fixtures.NAMED/'fixtures.py',fixtures.GENERIC/'fixtures.py',fixtures.GENERIC/'decoder_fixtures.py',HERE.parent.parent/'execution/fixtures.py',HERE.parent.parent/'to-integer-execution/fixtures.py',HERE.parent.parent.parent/'pipeline/fixtures.py']
...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix='cathedral-mid-public-')as d:
  ...
  target=Path('/tmp/cathedral-mid-public-target');command=[str(cargo),'build','--offline','--locked','--release','--manifest-path',str(work/'Cargo.toml'),'--target-dir',str(target)]
