#!/usr/bin/env python3
"""Reproduce source-bound component interaction evidence from an explicit Git checkpoint.
...
DIRECTORY = 'tools/ports/acpi/interpreter/component-integration/'
...
def main():
    ...
    record = json.loads(blob(DIRECTORY + 'verification.json'))
    ...
    assert record['group'] == 'component integration' and record['match'] == ''
    sys.dont_write_bytecode = True
    with tempfile.TemporaryDirectory(prefix='cathedral-component-history-') as folder:
        archive = Path(folder)
        ...
        spec = importlib.util.spec_from_file_location('archived_component_check', source)
        check = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(check)
        assert check.snapshot() == record['source_sha256']
        rows, generated, selections = check.fixtures.render('')
        ...
        print('PASS historical component integration', len(rows), 'pairs;',
