#!/usr/bin/env python3
"""Check and execute real Omega byte-concat bodies and changed-body controls."""
...
def main():
 ...
 before=snapshot()
 with tempfile.TemporaryDirectory(prefix='cathedral-acpi-byte-concat-')as directory:
  ...
  if a.record:a.record.write_text(json.dumps({'format':'cathedral-acpi-byte-concat-checked-v1','stage':'checked-interpreter execution; native/hardware not run','omega_revision':revision,'runner_sha256':hashlib.sha256(runner.read_bytes()).hexdigest(),'suite_sha256':hashlib.sha256(source.encode()).hexdigest(),'scenario_count':len(rows),'control_count':len(rows),'cases':[r['name']for r in rows],'evaluator_step_limit':10000000,'elapsed_seconds':round(time.monotonic()-start,3),'source_sha256':before,'output':''.join(lines)},indent=2,sort_keys=True)+'\n')
 print(f'PASS {len(rows)} byte-concat bodies + {len(rows)} changed-body controls. Native/hardware NOT RUN.')
