...
selected=['integer_4_2','buffer_halves','string_invalid_beats_capacity']
rows={r['name']:r for r in fixtures.cases()};runs=[]
for name in selected:
 for control in [False,True]:
  with tempfile.TemporaryDirectory(prefix='cathedral-byte-concat-const-')as directory:
...
assert snapshot()==before,'Source changed during constant run'
record={'stage':'constant evaluation and checked requires; native/hardware not run','compiler_sha256':hashlib.sha256(compiler.read_bytes()).hexdigest(),'cases':selected,'runs':runs,'source_sha256':before}
