...
def module_source(group, selected):
    ...
    receiver = ''.join(part.title() for part in group.split('_'))+'ToStringSuite'
...
def build_text(root=ROOT):
    source = 'machine build(builder:&mut Build){builder.application("cathedral-to-string-execution");builder.freestanding=true;'
...
def main():
    ...
    def run_batch(batch):
        ...
        with tempfile.TemporaryDirectory(prefix='cathedral-to-string-execution-') as directory:
            work=Path(directory)
