#!/usr/bin/env python3
"""Run ObjectType integration through actual Omega bodies and bind exact inputs."""
import argparse
import hashlib
import json
import os
...
def build(root):
    source = 'machine build(builder:&mut Build){builder.package("object-type-execution");builder.freestanding=true;'
...
def verify(path):
    ...
    rows, source, names = fixtures.render(record['group'], record['match'])
    assert record['omega_revision'] == PIN and record['evaluator_step_limit'] == 10000000
...
def main():
    ...
    p.add_argument('--group', choices=['execution','decoder'], default='execution')
    p.add_argument('--match', default='')
    p.add_argument('--runner', type=Path, default=RUNNER)
    p.add_argument('--omega-source', type=Path, default=Path('/tmp/cathedral-omega-source-eaa7993'))
    ...
    assert root == ROOT, 'Receipt snapshot must bind the executed source root'
    omega=args.omega_source.resolve()
    revision=subprocess.check_output(['git','rev-parse','HEAD'],cwd=omega,text=True).strip()
    assert revision == PIN, 'Wrong Omega source revision'
    assert not subprocess.check_output(['git','status','--porcelain'],cwd=omega,text=True).strip(), 'Omega source must be clean'
    ...
    with tempfile.TemporaryDirectory(prefix='cathedral-object-type-execution-') as directory:
        ...
        print('Runner PID:',process.pid,flush=True)
        lines=[]
        ...
        record=dict(omega_revision=revision,omega_source=str(omega),evaluator_step_limit=10000000,production_root=str(root),group=args.group,match=args.match,
    ...
    print('PASS',len(rows),args.group,'behavior/control pairs; native/hardware NOT RUN')
