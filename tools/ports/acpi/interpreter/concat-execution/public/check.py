#!/usr/bin/env python3
"""Pinned public Concatenate observations using the unchanged inert service-trap harness."""
...
def describe(value):
 kind,data=value
 return 'integer:'+str(data) if kind in ('integer','inline') else ('string:' if kind=='string' else 'buffer:')+data.hex()
def selection():
 import re
 selected=[];omitted={}
 for row in fixtures.cases():
  if 'bytecode' not in row:
   omitted[row['name']]='Whole-state retirement setup has no equivalent public AML-only fixture.'
   continue
  # Recover the exact generated input array used by the checked AML assertion.
  body=fixtures.bytecode_body(row,False)
  length=int(re.search(r'prepare_program\(input,(\d+),',body).group(1));data=bytearray(length)
  for index,value in re.findall(r'input\[(\d+)\]=(\d+);',body):data[int(index)]=int(value)
  kind,output=fixtures.result(row['a'],row['b'],row['size'])
  if row['bytecode']=='nested':kind,output=fixtures.result((kind,output),('inline',3),row['size'])
  left=(kind,output) if row['bytecode']=='self' else row['a']
  selected.append(dict(name=row['name'],table=data.hex(),bits=row['size'],error='Success',kind=kind,bytes=output.hex(),after={'LEFT':describe(left),'RGHT':describe(row['b'])}))
 return selected,omitted
def expected(row):
 return {'result':row['kind']+':'+row['bytes'],**{'after:'+key:value for key,value in row['after'].items()}}
...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix='cathedral-concat-public-')as d:
  ...
  target=Path('/tmp/cathedral-concat-public-target');command=[str(cargo),'build','--offline','--locked','--release','--manifest-path',str(work/'Cargo.toml'),'--target-dir',str(target)]
