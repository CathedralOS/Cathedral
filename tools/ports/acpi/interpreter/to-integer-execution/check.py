#!/usr/bin/env python3
"""Run ToInteger integration through actual Omega bodies and bind exact inputs."""
import argparse
import hashlib
import json
import os
...
def build(root):
    source = 'machine build(builder:&mut Build){builder.package("to-integer-execution");builder.freestanding=true;'
...
def snapshot():
    ...
    paths += [fixtures.NAMED/'fixtures.py']
...
def main():
    ...
    p.add_argument('--group', choices=['execution','bridge'], default='execution')
    ...
    with tempfile.TemporaryDirectory(prefix='cathedral-to-integer-execution-') as directory:
        ...
        record=dict(omega_revision=PIN,production_root=str(root),group=args.group,match=args.match,
