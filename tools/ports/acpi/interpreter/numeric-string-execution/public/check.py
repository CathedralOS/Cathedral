#!/usr/bin/env python3
"""Pinned public numeric-string opcode observations using the unchanged service-trap driver."""
...
def selection():
    ...
    selected, omitted = [], {}
    for row in rows:
        assert row["kind"] in (1, 3) and row["bits"] in (32, 64)
        ...
        elif "_debug_target_" in row["name"]:
...
def expected(row):
    ...
    # Public names expose value/type effects, not Cathedral allocation identities.
    for block in row["owned_after"]:
        if block["object_id"] in (0, 1):
            after["SRC" if block["object_id"] == 0 else "DST"] = "string:" + block["bytes"]
    value = "integer:" + str(row["number"]) if row["kind"] == 1 else "string:" + row["bytes"]
    return {"result": value, **{"after:" + key: value for key, value in after.items()}}
...
def main():
    ...
    parser.add_argument("--target-dir", type=Path, default=Path("/tmp/cathedral-numeric-string-public-target"))
    ...
    with tempfile.TemporaryDirectory(prefix="cathedral-numeric-string-public-") as directory:
        ...
        for row in rows:
            ...
            run = subprocess.run([str(binary), str(aml), "1" if row["bits"] == 32 else "2", "", *[key[6:] for key in expected(row) if key.startswith("after:")]],
    ...
    record = dict(stage="Pinned public Interpreter load/evaluate; no native Omega or hardware claim",
        agreement_scope="Result value/type and named post-state only; no Cathedral allocation-ID or byte-padding observation",
