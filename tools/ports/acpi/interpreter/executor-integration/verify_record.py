...
def verify(record, require_binaries=False, require_complete=False):
    ...
    assert record['scope'] in {'full', 'selected'}
    assert record['groups'] == [selection['group'] for selection in record['groups_selected']]
    assert record['groups'] and len(record['groups']) == len(set(record['groups']))
    assert set(record['groups']) <= set(check.GROUPS)
    assert type(record['combine_groups']) is bool
    if require_complete:
    ...
    for selection in record['groups_selected']:
        ...
        assert selection['cases'] and len(selection['cases']) == len(set(selection['cases']))
        assert selection['cases'] == [row['name'] for row in rows if row['name'] in selection['cases']]
        if record['scope'] == 'full':
            assert selection['cases'] == [row['name'] for row in rows]
        count+=len(selection['cases'])
    expected_batches=check.plan_batches(record['groups_selected'],record['batch_size'],record['combine_groups'])
    assert len(expected_batches)==len(record['batches'])
    for plan,batch in zip(expected_batches,record['batches']):
        sources,modules,names,driver=check.render_package(plan)
        assert batch['modules']==modules and names==batch['selections']
        assert check.text_sha(driver)==batch['driver_sha256']
...
def main():
    ...
    parser.add_argument('--require-complete', action='store_true', help='Require every declared group and row, not a narrowed full group.')
    args = parser.parse_args()
    record = json.loads(args.record.read_text())
    count = verify(record,args.require_binaries,args.require_complete)
