...
def main():
 ...
 for p,h in baseline.items():assert check.sha(ROOT/p)==h,'copied canonical file modified: '+p
 print('PASS',len(rows),'literal preflight pairs,3constant pairs,',len(host['rows']),'public observations; copied canonical sources unchanged')
