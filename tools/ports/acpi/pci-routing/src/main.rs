...
fn main(){
 ...
 let args:Vec<String>=std::env::args().collect();let source=std::fs::read(&args[1]).unwrap();
 ...
 let run=catch_unwind(AssertUnwindSafe(||{
  ...
  let path=AmlName::from_str(&args[2]).unwrap();
  let table=match catch_unwind(AssertUnwindSafe(||PciRoutingTable::from_prt_path(path,&instance))){
   Ok(Ok(table))=>{println!("decode\tok:{table:?}");table},
   Ok(Err(e))=>{println!("decode\terror:{e:?}");return},
   Err(_)=>{println!("decode\tpanic");return}
  };
  for (index,query) in args[3..].iter().enumerate(){
   let parts:Vec<u16>=query.split(',').map(|s|s.parse().unwrap()).collect();
   let pin=match parts[2]{0=>Pin::IntA,1=>Pin::IntB,2=>Pin::IntC,3=>Pin::IntD,_=>panic!("invalid probe pin")};
   match catch_unwind(AssertUnwindSafe(||table.route(parts[0],parts[1],pin,&instance))){
    Ok(Ok(irq))=>println!("route:{index}\tok:{irq:?}"),
    Ok(Err(e))=>println!("route:{index}\terror:{e:?}"),
    Err(_)=>println!("route:{index}\tpanic")
   }
  }
 }));
 if run.is_err(){println!("load_or_probe\tpanic")}
