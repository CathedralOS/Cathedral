...
impl Traps {
 ...
 fn write(&self,address:usize,width:usize,value:u64) {
  let offset=address.checked_sub(0x1000).expect("synthetic base");
  self.log.lock().unwrap().push(format!("write,{offset},{width},{value}"));
  self.memory.lock().unwrap()[offset..offset+width].copy_from_slice(&value.to_le_bytes()[..width]);
 }
}
impl Handler for Traps {
 ...
 fn write_u8(&self,a:usize,v:u8){self.write(a,1,v as u64)}
 fn write_u16(&self,a:usize,v:u16){self.write(a,2,v as u64)}
 fn write_u32(&self,a:usize,v:u32){self.write(a,4,v as u64)}
 fn write_u64(&self,a:usize,v:u64){self.write(a,8,v)}
...
fn main(){
 ...
 let handler=Traps::default();*handler.memory.lock().unwrap()=(0..512).map(|i|((i*37+11)&255) as u8).collect();
