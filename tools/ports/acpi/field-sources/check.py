#!/usr/bin/env python3
"""Execute actual bounded payload sequencing bodies against independent interval vectors."""
...
def snapshot():
 ...
 paths += list((ROOT/'source/libraries/acpi/field_sources').glob('*.omg'))
 paths += [ROOT/'source/libraries/acpi/aml'/name for name in ['build.omg','byte_storage.omg','bytes.omg','model.omg','names.omg','namespace.omg','object_conversions.omg','object_references.omg']]
...
def build_text():
 text='machine build(builder:&mut Build){builder.application("cathedral-field-sources-tests");builder.freestanding=true;'
 for alias,folder in [('sources','field_sources'),('integers','interpreter'),('aml','aml')]:text+='builder.depend_as("'+alias+'",Source::Path {location:"'+str(ROOT/'source/libraries/acpi'/folder)+'"});'
 return text+'}'
...
def main():
 ...
 def run_batch(selected):
  ...
  with tempfile.TemporaryDirectory(prefix='cathedral-field-sources-checked-')as d:
