...
def main():
 p=argparse.ArgumentParser();p.add_argument('--repository',type=Path,default=check.ROOT);p.add_argument('--checked-only',action='store_true');p.add_argument('--record',default='checked-verification.json');a=p.parse_args()
 ...
 assert record['build_sha256']==hashlib.sha256(check.build_text().encode()).hexdigest()
 assert record['batch_elapsed_seconds_sum']==round(sum(b['elapsed_seconds']for b in record['batches']),3)
 assert 1<=record['workers']<=3
 ...
 checkout=a.repository.resolve()/'reference_code/rust-osdev/acpi'
 assert subprocess.check_output(['git','rev-parse','HEAD'],cwd=checkout,text=True).strip()==reference.PIN
 for relative,digest in public['upstream_sha256'].items():assert check.sha(checkout/relative)==digest,relative
 ...
 if not a.checked_only:
  ...
  assert const['build_sha256']==hashlib.sha256(check.build_text().encode()).hexdigest()
