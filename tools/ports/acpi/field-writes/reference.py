...
def cases():
 return [dict(name=row['name'],offset=row['offset'],length=row['length'],flags=row['flags'],region=row['region'],revision=1 if row['size']==4 else 2,source=row['public_source'],strict_expected=row['expected'])for row in vectors.public_cases()]
...
def main():
 ...
 target=Path('/tmp/cathedral-field-writes-public')
 with tempfile.TemporaryDirectory(prefix='field-writes-reference-')as d:
 ...
 print('PASS',len(rows),'public field write observations')
