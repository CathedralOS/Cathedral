...
def rows(group):
    return (fixtures if group == 'chunk' else fixtures.bulk).cases()


def module_source(group, selected):
    source, entries = (fixtures if group == 'chunk' else fixtures.bulk).render(selected)
...
def main():
    ...
    assert json.loads((HERE/'cases.json').read_text()) == rows('chunk')
    ...
    def run_batch(batch):
        ...
        with tempfile.TemporaryDirectory(prefix='cathedral-field-write-chunks-') as directory:
            work=Path(directory)
