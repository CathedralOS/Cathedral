...
s=(C/'values-buffer.omg').read_text().replace('use aml::model::Value;','use aml::model::Value;\nuse aml::model::BufferStorage;')+'''\nmachine buffer_is(value:Value)->bool {transition value {Value::Buffer {buffer_storage} -> storage(buffer_storage) _ -> (false)} state storage(value:BufferStorage)->bool {transition value {BufferStorage::Source {declared_size,buffer_initializer} -> check(declared_size,buffer_initializer) _ -> (false)}} state check(size:u64,initializer:Span)->bool {size==2 && initializer.unit==19 && initializer.start==4 && initializer.end==8}}\n''';(C/'values-buffer.omg').write_text(s)
# Loader integration.
...
source=(C/'string-validation.omg').read_text().replace('use aml::model::Value;','use aml::model::Value;\nuse aml::model::StringStorage;')+"\nmachine string_is(value:Value)->bool {transition value {Value::String {string_storage} -> storage(string_storage) _ -> (false)} state storage(value:StringStorage)->bool {transition value {StringStorage::Source {string_source} -> check(string_source) _ -> (false)}} state check(bytes:Span)->bool {bytes.unit==3 && bytes.start==1 && bytes.end==2}}\n";(C/'string-validation.omg').write_text(source)
