...
def authored(rows):
 ...
 return text,names
def build():return 'machine build(builder:&mut Build){builder.application("cathedral-implicit-dispatch-checks");builder.freestanding=true;builder.depend_as("aml",Source::Path {location:"'+str(ROOT/'source/libraries/acpi/aml')+'"});builder.depend_as("integer_helpers",Source::Path {location:"'+str(ROOT/'source/libraries/acpi/interpreter')+'"});}'
SOURCE=['source/libraries/acpi/aml/build.omg', 'source/libraries/acpi/aml/byte_storage.omg', 'source/libraries/acpi/aml/bytes.omg', 'source/libraries/acpi/aml/model.omg', 'source/libraries/acpi/aml/names.omg', 'source/libraries/acpi/aml/namespace.omg', 'source/libraries/acpi/aml/object_conversions.omg', 'source/libraries/acpi/aml/object_references.omg', 'source/libraries/acpi/interpreter/buffer_fields.omg', 'source/libraries/acpi/interpreter/build.omg', 'source/libraries/acpi/interpreter/conversions.omg', 'source/libraries/acpi/interpreter/integers.omg', 'source/libraries/acpi/interpreter/string_numbers.omg', 'source/libraries/acpi/interpreter/implicit_integer.omg', 'source/libraries/acpi/interpreter/implicit_strings.omg', 'source/libraries/acpi/aml/implicit_conversions.omg']
def snapshot():
...
def selected(rows):return [next(r for r in rows if r['name']==n)for n in ['string_64_Integer_True_1234','number_32_String_18446744073709551615','buffer_capacity_85_True_String']]
def const_source(row,control):return IMPORTS+HELPERS+'machine ic_constant_result()->i32{'+body(row,control)+'}\nconst RESULT:i32=ic_constant_result();\nmachine require_ok(value:i32) requires value==0;{}\ndata Main{}\nmachine Main::main(&mut self){require_ok(RESULT);}\n'
def run():
 ...
 with tempfile.TemporaryDirectory(prefix='cathedral-implicit-dispatch-')as d:
