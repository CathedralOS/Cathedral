...
def selected(rows):return [next(r for r in rows if r['name']==n)for n in ['first','last_slot','late_dangling']]
def const_source(row,control):return IMPORTS+HELPERS+'machine package_constant_result()->i32{'+body(row,control).replace('=check(', '=const_check(')+'}\nconst RESULT:i32=package_constant_result();\nmachine require_ok(value:i32) requires value==0;{}\ndata Main{}\nmachine Main::main(&mut self){require_ok(RESULT);}\n'
def checked_batch(batch):
 text,selections=authored(batch);start=time.monotonic()
 with tempfile.TemporaryDirectory(prefix='cathedral-package-index-batch-')as d:
...
def run():
 ...
 with tempfile.TemporaryDirectory(prefix='cathedral-package-index-const-')as d:
