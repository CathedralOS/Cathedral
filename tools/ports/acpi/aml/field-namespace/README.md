...
Canonical integration retained the complete tested source ancestor `24aed04`.
The isolated 38 loader pairs and constant pair retain their original receipts.
`integration-reconciliation.json` lists the Buffer Store, executor and Program
inputs that differ, plus auxiliary constant-fixture history; it is not a rerun.
All 20 unchanged parser/consumer/FieldUnit boundary pairs subsequently passed on
the combined canonical source, with exact receipt verification:

```sh
python3 tools/ports/acpi/aml/fields/migration.py --record tools/ports/acpi/aml/fields/integrated-migration-verification.json --verify
```

The sole production merge conflict retained named Store conversion and added
the FieldUnit UnresolvedRegion case. Shared parser/model ownership, loader
transactionality and existing primary Field metadata were preserved.
