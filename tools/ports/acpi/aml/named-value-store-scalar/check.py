...
def authored(rows):
 ...
 return text,names
def build(root=ROOT):return 'machine build(builder:&mut Build){builder.application("cathedral-named-value-store-scalar-checks");builder.freestanding=true;builder.depend_as("aml",Source::Path {location:"'+str(root/'source/libraries/acpi/aml')+'"});builder.depend_as("integer_helpers",Source::Path {location:"'+str(root/'source/libraries/acpi/interpreter')+'"});}'
SOURCE=['source/libraries/acpi/aml/build.omg', 'source/libraries/acpi/aml/model.omg', 'source/libraries/acpi/aml/bytes.omg', 'source/libraries/acpi/aml/names.omg', 'source/libraries/acpi/aml/namespace.omg', 'source/libraries/acpi/aml/object_references.omg', 'source/libraries/acpi/aml/byte_storage.omg', 'source/libraries/acpi/aml/object_conversions.omg', 'source/libraries/acpi/aml/implicit_conversions.omg', 'source/libraries/acpi/aml/buffer_target_values.omg', 'source/libraries/acpi/aml/named_value_store.omg', 'source/libraries/acpi/interpreter/build.omg', 'source/libraries/acpi/interpreter/integers.omg', 'source/libraries/acpi/interpreter/conversions.omg', 'source/libraries/acpi/interpreter/buffer_fields.omg', 'source/libraries/acpi/interpreter/implicit_integer.omg', 'source/libraries/acpi/interpreter/implicit_strings.omg', 'source/libraries/acpi/interpreter/string_numbers.omg']
def snapshot():
 paths=[HERE/n for n in ['check.py','fixtures.py','cases.json','baseline_fixtures.py','baseline_cases.json']]+[ROOT/p for p in SOURCE]+[ROOT/'tools/ports/acpi/interpreter/execution'/n for n in ['checked_runner.rs','runner.Cargo.lock']]
...
def selected(rows):return [next(r for r in rows if r['name']==n)for n in ['integer_reset','string_self_owned','destination_before_source','scalar_integer_32_18446744073709551615','scalar_string_64_True_3_18446744073709551615','scalar_buffer_32_True_9_18446744073709551615']]
def const_source(row,control):return IMPORTS+HELPERS+'machine named_store_constant_result()->i32{'+body(row,control).replace('=check(', '=const_check(').replace('=check_scalar(', '=const_check_scalar(')+'}\nconst RESULT:i32=named_store_constant_result();\nmachine require_ok(value:i32) requires value==0;{}\ndata Main{}\nmachine Main::main(&mut self){require_ok(RESULT);}\n'
def checked_batch(batch):
 text,selections=authored(batch);start=time.monotonic()
 with tempfile.TemporaryDirectory(prefix='cathedral-named-value-store-scalar-batch-')as d:
...
def run():
 ...
 (HERE/'verification.json').write_text(json.dumps(dict(omega_revision=PIN,input_sha256=inputs,runner_sha256=runner,compiler_sha256=compiler,source_sha256=hashlib.sha256(source.encode()).hexdigest(),execution_root=str(ROOT),build_text=build(),build_sha256=hashlib.sha256(build().encode()).hexdigest(),workers=3,batch_seconds_sum=round(sum(b['seconds']for b in batches),3),positive_count=len(rows),control_count=len(rows),batches=batches,constant_proofs=proofs,seconds=round(time.monotonic()-start,3)),indent=2,sort_keys=True)+'\n')
 with tempfile.TemporaryDirectory(prefix='cathedral-named-value-store-scalar-const-')as d:
...
def verify():
 ...
 assert r['execution_root']==str(ROOT.resolve()) and r['build_text']==build(Path(r['execution_root'])) and r['build_sha256']==hashlib.sha256(r['build_text'].encode()).hexdigest()
 ...
 assert r['positive_count']==r['control_count']==len(rows)and r['source_sha256']==hashlib.sha256(source.encode()).hexdigest();assert len(r['constant_proofs'])==12
 ...
 print('PASS',len(rows),'checked pairs and 6 const pairs; exact current hashes')
