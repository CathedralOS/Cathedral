...
def main():
 ...
 assert runtime['runner_sha256']==check.sha(Path('/tmp/cathedral-acpi-object-concat-described-runner/release/cathedral-acpi-checked-runner'))
 ...
 assert constant['cases']==['desc_Device_buffer_left','desc_Package_integer_right','left_encoding_before_right_id_True'];assert len(constant['proofs'])==6
 ...
 if history.exists():
  prior=json.loads((history/'tools/ports/acpi/aml/object-concat-described/manifest.json').read_text())
  for path,digest in prior['files_sha256'].items():assert check.sha(history/path)==digest,'historical artifact changed: '+path
 print('PASS',len(rows),'description-aware concatenation behavior/control pairs, 3 constant pairs and 52 actual public opcode observations; exact closure verified')
