...
def authored(rows):
 ...
 return text,names
def build(root=ROOT):return 'machine build(builder:&mut Build){builder.application("cathedral-buffer-target-values-checks");builder.freestanding=true;builder.depend_as("aml",Source::Path {location:"'+str(root/'source/libraries/acpi/aml')+'"});builder.depend_as("integer_helpers",Source::Path {location:"'+str(root/'source/libraries/acpi/interpreter')+'"});}'
SOURCE=['source/libraries/acpi/aml/build.omg', 'source/libraries/acpi/aml/byte_storage.omg', 'source/libraries/acpi/aml/bytes.omg', 'source/libraries/acpi/aml/model.omg', 'source/libraries/acpi/aml/names.omg', 'source/libraries/acpi/aml/namespace.omg', 'source/libraries/acpi/aml/object_conversions.omg', 'source/libraries/acpi/aml/object_references.omg', 'source/libraries/acpi/aml/buffer_target_values.omg', 'source/libraries/acpi/interpreter/build.omg', 'source/libraries/acpi/interpreter/buffer_fields.omg', 'source/libraries/acpi/interpreter/conversions.omg', 'source/libraries/acpi/interpreter/integers.omg', 'source/libraries/acpi/interpreter/string_numbers.omg','source/libraries/acpi/aml/implicit_conversions.omg','source/libraries/acpi/interpreter/implicit_integer.omg','source/libraries/acpi/interpreter/implicit_strings.omg']
def snapshot():
...
def selected(rows):return [next(r for r in rows if r['name']==n)for n in ['integer_32_18446744073709551615_9','string_1_256_256','empty_0']]
def const_source(row,control):return IMPORTS+HELPERS+'machine btv_constant_result()->i32{'+body(row,control)+'}\nconst RESULT:i32=btv_constant_result();\nmachine require_ok(value:i32) requires value==0;{}\ndata Main{}\nmachine Main::main(&mut self){require_ok(RESULT);}\n'
def checked_batch(batch):
 text,selections=authored(batch);start=time.monotonic()
 with tempfile.TemporaryDirectory(prefix='cathedral-buffer-target-values-batch-')as d:
...
def run():
 ...
 with tempfile.TemporaryDirectory(prefix='cathedral-buffer-target-values-const-')as d:
