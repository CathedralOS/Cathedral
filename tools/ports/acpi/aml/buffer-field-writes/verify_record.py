...
def main():
 ...
 assert runtime['runner_sha256']==check.sha(Path('/tmp/cathedral-acpi-buffer-field-write-runner/release/cathedral-acpi-checked-runner'))
 ...
 assert constant['cases']==['write_9_1_False','string_nul_True','field_max'];assert len(constant['proofs'])==6
 for proof in constant['proofs']:
  row=mapping[proof['case']];text=fixtures.IMPORTS+fixtures.HELPERS+fixtures.render(row,proof['control'],constant=True)+'const TEST_RESULT:i32=test_result();\nmachine require_ok(value:i32) requires value==0;{}\ndata Main{}\nmachine Main::main(&mut self){require_ok(TEST_RESULT);}\n'
 ...
 for row in host['rows']:
  ...
  if r['error']is None:assert observed['result']=='ok'and observed['after']==bytes(r['expected']).hex()
  if r['error']=='UnsupportedValue':assert observed['result'].startswith('error:')and observed['after']==bytes(r['logical']).hex()
 history=HERE/'history/prepublication'
 prior=json.loads((history/'tools/ports/acpi/aml/buffer-field-writes/manifest.json').read_text())
 for path,digest in prior['files_sha256'].items():assert check.sha(history/path)==digest,'historical artifact changed: '+path
 print('PASS',len(rows),'atomic field-write behavior/control pairs,3 constant pairs,',len(host['rows']),'actual public Object observations; exact source/tool closure verified')
