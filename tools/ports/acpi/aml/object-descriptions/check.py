#!/usr/bin/env python3
"""Execute direct object descriptions and record exact sources and dependency roots."""
import argparse,hashlib,json,os,subprocess,tempfile,time
from pathlib import Path
import fixtures
HERE=fixtures.HERE;ROOT=fixtures.ROOT;CANONICAL=fixtures.ROOT;SHARED=CANONICAL/'tools/ports/acpi/interpreter/execution'
PIN='eaa7993a23623cd8fabf45350340479c5c9c7879'
SOURCE=['source/libraries/acpi/aml/'+n for n in ['build.omg','model.omg','object_conversions.omg','object_descriptions.omg','bytes.omg','names.omg','namespace.omg','object_references.omg','byte_storage.omg']]+['source/libraries/acpi/interpreter/'+n for n in ['build.omg','integers.omg','conversions.omg','buffer_fields.omg','string_numbers.omg']]
def sha(path):return hashlib.sha256(path.read_bytes()).hexdigest()
def snapshot():
 paths=[ROOT/p for p in SOURCE]+list(HERE.glob('*.py'))+[HERE/'cases.json',HERE/'labels.json']+[SHARED/n for n in ['checked_runner.rs','runner.Cargo.lock']]
...
def build_text(stage):
 ...
 application='cathedral-object-descriptions'if stage=='runtime'else'object-description-constant'
 root=ROOT.resolve()
...
def build_runner(omega):
 ...
 target=Path('/tmp/cathedral-acpi-object-description-runner')
 with tempfile.TemporaryDirectory(prefix='cathedral-object-description-runner-')as d:
...
def main():
 p=argparse.ArgumentParser();p.add_argument('--omega-source',type=Path,default=CANONICAL.parent/'Omega');p.add_argument('--match',default='');p.add_argument('--batch',type=int,default=5);p.add_argument('--record',type=Path);a=p.parse_args();assert a.batch>0
 ...
 with tempfile.TemporaryDirectory(prefix='cathedral-object-descriptions-')as d:
  work=Path(d);(work/'build.omg').write_text(build);assert sha(work/'build.omg')==build_sha256
  for i in range(0,len(rows),a.batch):
   batch=rows[i:i+a.batch];text,names=source(batch);(work/'main.omg').write_text(text);assert sha(runner)==binary and sha(work/'build.omg')==build_sha256;start=time.monotonic()
   ...
   records.append(dict(execution_root=execution_root,build_sha256=build_sha256,cases=[r['name']for r in batch],source_sha256=hashlib.sha256(text.encode()).hexdigest(),runner_sha256=binary,seconds=round(time.monotonic()-start,3),output=output))
 if a.record:a.record.write_text(json.dumps(dict(execution_root=execution_root,build_text=build,build_sha256=build_sha256,stage='checked interpreter actual Omega bodies and changed-body controls; native/hardware not run',omega_revision=PIN,runner_sha256=binary,input_sha256=inputs,cases=[r['name']for r in rows],positive_count=len(rows),control_count=len(rows),batches=records),indent=2,sort_keys=True)+'\n')
 print('PASS',len(rows),'object description behavior/control pairs')
