...
Both execution stages record the resolved dependency root as `execution_root`,
the exact generated `build_text`, and `build_sha256`. Each runtime batch and
constant proof also records its root and build hash. The verifier reconstructs
the runtime/constant build configurations from the current resolved repository
root and rejects a scratch root, altered build text or hash. Generated build
files are checked before and after execution. Earlier completed receipts and
tools without this root binding are retained unchanged under history/root-unbound;
their original manifest is verified separately as historical evidence.
