#!/usr/bin/env python3
"""Validate current checked/constant source hashes and exact results, without rerun."""
...
def main():
 ...
 text,names=fixtures.render(rows);assert record['source_sha256']==hashlib.sha256(text.encode()).hexdigest()
 assert record['positive_count']==record['control_count']==len(rows)
 assert record['runner_sha256']==check.sha(Path(record['runner_path']));check.validate(record['output'],names)
 ...
 assert [{key:r[key]for key in ['kind','text_hex']}for r in public['observations']]==reference.cases()
 if not a.checked_only:
  ...
  assert const['positive_count']==const['control_count']==len(check_const.selected());assert len(const['proofs'])==2*len(check_const.selected())
  assert const['compiler_sha256']==check.sha(Path(const['compiler_path']))
 ...
 print('PASS',len(rows),'checked pairs;',len(record['input_sha256']),'current hashes;',len(public['observations']),'bound public observations;','checked only'if a.checked_only else'constant pairs verified')
