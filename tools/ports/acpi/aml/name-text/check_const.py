#!/usr/bin/env python3
"""Representative actual text bodies evaluated as constants, with body controls."""
...
def selected():
 rows=fixtures.cases();lower=next(row for row in rows if row.get('input_hex')==b'\\a.b0._c.dEf'.hex());return [lower,next(row for row in rows if row['name']=='format_0_16_16'),next(row for row in rows if row['name']=='guard_valid_count_18446744073709551615')]
def source(row,control):return fixtures.IMPORTS+fixtures.HELPERS+'machine test_result()->i32 {\n'+fixtures.body(row,control)+'}\nconst TEST_RESULT:i32=test_result();\nmachine require_ok(value:i32) requires value==0;{}\ndata Main{}\nmachine Main::main(&mut self){require_ok(TEST_RESULT);}\n'
def main():
 ...
 for row in selected():
  for control in [False,True]:
   ...
   with tempfile.TemporaryDirectory(prefix='cathedral-name-text-const-')as d:
 ...
 (HERE/'const-verification.json').write_text(json.dumps(dict(stage='constant evaluation and requires proof; no native or namespace evaluation',omega_revision=check.PIN,input_sha256=inputs,compiler_sha256=binary,compiler_path=str(a.compiler),positive_count=len(selected()),control_count=len(selected()),elapsed_seconds=round(time.monotonic()-start,3),proofs=proofs),indent=2,sort_keys=True)+'\n')
