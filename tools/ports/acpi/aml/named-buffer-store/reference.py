...
def expected_rows():
    rows = []
    for extent,n in [(n,n) for n in [0,1,3,8,255,256]]+[(0,1),(1,0),(1,3),(3,1),(255,256),(256,255)]:
        data = bytes((i*37+128)%256 for i in range(n))
        rows.append(dict(target_extent=extent,source_hex=data.hex(),
                         output='result\tbuffer:'+str(n)+':'+data.hex()+'\n',
                         classification='equal-extent-match' if extent==n else 'excluded-unequal-extent-pin-resizes'))
    return rows
...
def main():
    ...
    if args.verify_record:
        ...
        print('PASS 12 public observations; exact current inputs and probe binary')
        return
    with tempfile.TemporaryDirectory(prefix='named-buffer-store-public-') as directory:
        work = Path(directory)
        ...
        for row in expected_rows():
            result = subprocess.run([str(binary),'Buffer',row['source_hex'],str(row['target_extent'])],capture_output=True,text=True,check=True)
        ...
        receipt = dict(pin=PIN, execution_root=str(ROOT), checkout=str(upstream), build_text=manifest,
                       ...
                       notes=['No interpreter, source/target aliasing, unsafe ObjectToken, provider or device calls.',
                              'Unequal-extent observations document the pin; they do not assert normative Store extent semantics.'])
        args.record.write_text(json.dumps(receipt,indent=2,sort_keys=True)+'\n')
    print('PASS',len(rows),'actual public pinned Buffer replacement observations')
