...
def selected(rows):return [next(r for r in rows if r['name']==n)for n in ['matrix_32_And_2_1','matrix_64_Less_1_2','unary_64_4']]
def const_source(row,control):return IMPORTS+HELPERS+'machine ol_constant_result()->i32{'+body(row,control)+'}\nconst RESULT:i32=ol_constant_result();\nmachine require_ok(value:i32) requires value==0;{}\ndata Main{}\nmachine Main::main(&mut self){require_ok(RESULT);}\n'
def checked_batch(batch):
 text,selections=authored(batch);start=time.monotonic()
 with tempfile.TemporaryDirectory(prefix='cathedral-object-logic-batch-')as d:
...
def run():
 ...
 with tempfile.TemporaryDirectory(prefix='cathedral-object-logic-const-')as d:
