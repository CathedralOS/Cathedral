...
assert r['binary_sha256']==reference.sha(Path('/tmp/cathedral-buffer-field-store-public/release/reference'))
expected=reference.cases();assert len(r['rows'])==len(expected)==116
...
assert {k:sum(row['classification']==k for row in r['rows'])for k in ['matches','raw-integer-width','string-source-panic']}=={'matches':82,'raw-integer-width':10,'string-source-panic':24}
print('PASS 116 public Store observations:82matching,10width differences,24String panics; exact current inputs/root/build and27upstream hashes')
