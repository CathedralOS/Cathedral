...
fn main(){
 ...
 println!("before\t{}",describe(&backing));println!("argument_before\t{}",describe(&argument));
 ...
 let after=instance.namespace.lock().get(AmlName::from_str("\\BACK").unwrap()).unwrap();
 println!("after\t{}",describe(&after));println!("same_backing\t{}",std::ptr::eq(&*backing,&*after));
 ...
 if args[5]=="alias" {
  ...
  println!("source_distinct_backing\t{}",!std::ptr::eq(&*named,&*after));
 }
