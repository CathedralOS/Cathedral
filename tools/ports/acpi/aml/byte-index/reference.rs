...
fn main(){
 ...
 let result=catch_unwind(AssertUnwindSafe(||{
  ...
  let backing=instance.namespace.lock().get(AmlName::from_str("\\BACK").unwrap()).unwrap();
  ...
  match (first,second) {
   (Ok(a),Ok(b))=>match (&*a,&*b) {
    (Object::Reference{kind,inner},Object::Reference{kind:other,inner:again})=>{
     ...
     println!("fresh_wrappers\t{}",!std::ptr::eq(&*a,&*b)&&!std::ptr::eq(&*a,&**inner));
     println!("fresh_fields\t{}",!std::ptr::eq(&**inner,&**again)&&!std::ptr::eq(&**inner,&*backing));
     match (&**inner,&**again) {
      (Object::BufferField{buffer,offset,length},Object::BufferField{buffer:other,offset:second_offset,length:second_length})=>{
       println!("same_backing\t{}",std::ptr::eq(&**buffer,&*backing)&&std::ptr::eq(&**other,&*backing));
       println!("geometry\t{offset}:{length}|{second_offset}:{second_length}");
      },_=>panic!("not fields")
     }
