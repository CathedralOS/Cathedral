...
def authored(rows):
 ...
 return text,names
def build(root=ROOT):return 'machine build(builder:&mut Build){builder.application("cathedral-byte-index-checks");builder.freestanding=true;builder.depend_as("aml",Source::Path {location:"'+str(root/'source/libraries/acpi/aml')+'"});builder.depend_as("integer_helpers",Source::Path {location:"'+str(root/'source/libraries/acpi/interpreter')+'"});}'
SOURCE=['source/libraries/acpi/aml/build.omg', 'source/libraries/acpi/aml/byte_storage.omg', 'source/libraries/acpi/aml/bytes.omg', 'source/libraries/acpi/aml/model.omg', 'source/libraries/acpi/aml/names.omg', 'source/libraries/acpi/aml/namespace.omg', 'source/libraries/acpi/aml/object_references.omg', 'source/libraries/acpi/interpreter/buffer_fields.omg', 'source/libraries/acpi/interpreter/build.omg', 'source/libraries/acpi/interpreter/conversions.omg', 'source/libraries/acpi/interpreter/integers.omg']
def snapshot():
 paths=[HERE/n for n in ['check.py','fixtures.py','store_checks.py','cases.json']]+[ROOT/p for p in SOURCE]+[ROOT/'tools/ports/acpi/interpreter/execution'/n for n in ['checked_runner.rs','runner.Cargo.lock']]
...
def selected(rows):return [next(r for r in rows if r['name']==n)for n in ['Buffer_False_8_7','last_two_slots','one_slot_partial_rollback']]
def const_source(row,control):return IMPORTS+HELPERS+'machine byte_index_constant_result()->i32{'+body(row,control).replace('=bi_check(', '=bi_const_check(')+'}\nconst RESULT:i32=byte_index_constant_result();\nmachine require_ok(value:i32) requires value==0;{}\ndata Main{}\nmachine Main::main(&mut self){require_ok(RESULT);}\n'
def checked_batch(batch):
 text,selections=authored(batch);start=time.monotonic()
 with tempfile.TemporaryDirectory(prefix='cathedral-byte-index-batch-')as d:
...
def run():
 ...
 with tempfile.TemporaryDirectory(prefix='cathedral-byte-index-const-')as d:
...
def verify():
 ...
 assert r['execution_root']==str(ROOT.resolve()) and r['build_text']==build()
 assert r['build_sha256']==hashlib.sha256(build().encode()).hexdigest()
