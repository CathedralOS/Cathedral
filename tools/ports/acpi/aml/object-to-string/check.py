...
def authored(rows):
 ...
 return text,names
def build(root=ROOT):return 'machine build(builder:&mut Build){builder.application("cathedral-object-to-string-checks");builder.freestanding=true;builder.depend_as("aml",Source::Path {location:"'+str(root/'source/libraries/acpi/aml')+'"});builder.depend_as("integer_helpers",Source::Path {location:"'+str(root/'source/libraries/acpi/interpreter')+'"});}'
SOURCE=['source/libraries/acpi/aml/build.omg', 'source/libraries/acpi/aml/byte_storage.omg', 'source/libraries/acpi/aml/bytes.omg', 'source/libraries/acpi/aml/model.omg', 'source/libraries/acpi/aml/names.omg', 'source/libraries/acpi/aml/namespace.omg', 'source/libraries/acpi/aml/object_conversions.omg', 'source/libraries/acpi/aml/object_references.omg', 'source/libraries/acpi/aml/object_to_string.omg', 'source/libraries/acpi/interpreter/build.omg', 'source/libraries/acpi/interpreter/buffer_fields.omg', 'source/libraries/acpi/interpreter/conversions.omg', 'source/libraries/acpi/interpreter/integers.omg', 'source/libraries/acpi/interpreter/string_numbers.omg']
def snapshot():
...
def selected(rows):return [next(r for r in rows if r['name']==n)for n in ['matrix_0_4_18446744073709551615','matrix_1_7_18446744073709551615','backing_capacity_0_18446744073709551615']]
def const_source(row,control):return IMPORTS+HELPERS+'machine ots_constant_result()->i32{'+body(row,control)+'}\nconst RESULT:i32=ots_constant_result();\nmachine require_ok(value:i32) requires value==0;{}\ndata Main{}\nmachine Main::main(&mut self){require_ok(RESULT);}\n'
def checked_batch(batch):
 text,selections=authored(batch);start=time.monotonic()
 with tempfile.TemporaryDirectory(prefix='cathedral-object-to-string-batch-')as d:
...
def run():
 ...
 with tempfile.TemporaryDirectory(prefix='cathedral-object-to-string-const-')as d:
