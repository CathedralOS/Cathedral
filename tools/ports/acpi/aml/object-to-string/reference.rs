...
fn describe(result:Result<acpi::aml::object::WrappedObject,AmlError>)->String {
 match result {Err(e)=>format!("error:{e:?}"),Ok(v)=>match &*v {Object::String(text)=>format!("string:{}",text.as_bytes().iter().map(|b|format!("{b:02x}")).collect::<String>()),v=>format!("other:{:?}",v.typ())}}
}
fn main(){
