#!/usr/bin/env python3
"""Original AML fixtures: semantic behavior plus body-mutating negative controls."""
import argparse
...
def main():
 ...
 def check(name):
  ...
  with tempfile.TemporaryDirectory(prefix='cathedral-aml-'+name+'-') as folder:
 ...
 if package_hash()!=before:raise SystemExit('AML source changed during checks; repeat against a fixed source hash.')
