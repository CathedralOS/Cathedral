...
def selected(rows):return [next(r for r in rows if r['name']==n)for n in ['integer_reset','string_self_owned','destination_before_source']]
def const_source(row,control):return IMPORTS+HELPERS+'machine named_store_constant_result()->i32{'+body(row,control).replace('=check(', '=const_check(')+'}\nconst RESULT:i32=named_store_constant_result();\nmachine require_ok(value:i32) requires value==0;{}\ndata Main{}\nmachine Main::main(&mut self){require_ok(RESULT);}\n'
def checked_batch(batch):
 text,selections=authored(batch);start=time.monotonic()
 with tempfile.TemporaryDirectory(prefix='cathedral-named-value-store-batch-')as d:
...
def run():
 ...
 with tempfile.TemporaryDirectory(prefix='cathedral-named-value-store-const-')as d:
...
def verify():
 ...
 assert r['build_text']==build(Path(r['execution_root'])) and r['build_sha256']==hashlib.sha256(r['build_text'].encode()).hexdigest()
