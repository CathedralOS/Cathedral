#!/usr/bin/env python3
"""Checked zero-length destination boundaries with exact source-bound receipts."""
import argparse
import ast
import hashlib
import json
...
from fixtures import BASE, EXTENT, HELPERS, IMPORTS, body, cases
...
def build():
    return ('machine build(builder:&mut Build){builder.application("cathedral-zero-buffer-store-checks");'
...
def checked_batch(rows):
    ...
    with tempfile.TemporaryDirectory(prefix='cathedral-zero-buffer-store-') as directory:
        work = Path(directory)
...
if __name__ == '__main__':
    ...
    groups = {'boundaries': ('cases.json', cases())}
    if args.write_cases:
        ...
        rows = groups['boundaries'][1]
        if args.match:
            rows = [r for r in rows if args.match in r['name']]
        assert rows
        record = args.record or HERE/'verification.json'
