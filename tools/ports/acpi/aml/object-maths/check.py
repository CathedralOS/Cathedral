...
SOURCE=['source/libraries/acpi/aml/build.omg', 'source/libraries/acpi/aml/byte_storage.omg', 'source/libraries/acpi/aml/bytes.omg', 'source/libraries/acpi/aml/model.omg', 'source/libraries/acpi/aml/names.omg', 'source/libraries/acpi/aml/namespace.omg', 'source/libraries/acpi/aml/object_conversions.omg', 'source/libraries/acpi/aml/object_references.omg', 'source/libraries/acpi/interpreter/buffer_fields.omg', 'source/libraries/acpi/interpreter/build.omg', 'source/libraries/acpi/interpreter/conversions.omg', 'source/libraries/acpi/interpreter/integers.omg', 'source/libraries/acpi/interpreter/string_numbers.omg', 'source/libraries/acpi/interpreter/implicit_integer.omg', 'source/libraries/acpi/interpreter/implicit_strings.omg', 'source/libraries/acpi/aml/implicit_conversions.omg', 'source/libraries/acpi/interpreter/bcd.omg', 'source/libraries/acpi/aml/object_maths.omg']
def snapshot():
...
def selected(rows):return [next(r for r in rows if r['name']==n)for n in ['matrix_32_Divide_0_2','unary_64_FromBcd_4','edge_32_ShiftLeft_4']]
def const_source(row,control):return IMPORTS+HELPERS+'machine omath_constant_result()->i32{'+body(row,control)+'}\nconst RESULT:i32=omath_constant_result();\nmachine require_ok(value:i32) requires value==0;{}\ndata Main{}\nmachine Main::main(&mut self){require_ok(RESULT);}\n'
def checked_batch(batch):
 text,selections=authored(batch);start=time.monotonic()
 with tempfile.TemporaryDirectory(prefix='cathedral-object-maths-batch-')as d:
...
def run():
 ...
 with tempfile.TemporaryDirectory(prefix='cathedral-object-maths-const-')as d:
