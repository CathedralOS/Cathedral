#!/usr/bin/env python3
"""Execute canonical object queries in isolated runner builds and record exact inputs."""
...
SOURCE=['source/libraries/acpi/aml/'+n for n in ['build.omg','model.omg','object_queries.omg','object_references.omg','namespace.omg','names.omg','bytes.omg','byte_storage.omg']]+['source/libraries/acpi/interpreter/'+n for n in ['build.omg','integers.omg','conversions.omg','buffer_fields.omg']]
...
def build_runner(omega):
 ...
 target=Path('/tmp/cathedral-acpi-object-query-runner')
 with tempfile.TemporaryDirectory(prefix='cathedral-query-runner-')as d:
...
def source(rows):
 ...
 for row in rows:
  for control in [False,True]:
   name='Suite::'+row['name']+('_control'if control else'_positive');text+=fixtures.render(row,control,name);names.append(name+'='+str(int(control)))
 return text,names

def main():
 p=argparse.ArgumentParser();p.add_argument('--omega-source',type=Path,default=ROOT.parent/'Omega');p.add_argument('--match',default='');p.add_argument('--batch',type=int,default=12);p.add_argument('--record',type=Path);a=p.parse_args();assert a.batch>0
 ...
 with tempfile.TemporaryDirectory(prefix='cathedral-object-queries-')as d:
  work=Path(d);(work/'build.omg').write_text('machine build(builder:&mut Build){builder.application("cathedral-object-queries");builder.freestanding=true;builder.depend_as("aml",Source::Path {location:"'+str(ROOT/'source/libraries/acpi/aml')+'"});}')
 ...
 print('PASS',len(rows),'query pairs')
