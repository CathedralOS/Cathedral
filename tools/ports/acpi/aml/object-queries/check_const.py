...
def main():
 inputs=check.snapshot();binary=check.sha(compiler);selected=['resolve_mixed_cycle','source_size_Buffer_4','namespace_bind_maximum'];rows={r['name']:r for r in fixtures.cases()};proofs=[]
...
''';(work/'main.omg').write_text(text);(work/'build.omg').write_text('machine build(builder:&mut Build){builder.application("query-constant");builder.freestanding=true;builder.depend_as("aml",Source::Path {location:"'+str(ROOT/'source/libraries/acpi/aml')+'"});}')
