...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix='cathedral-conversion-public-')as directory:
  ...
  for r in selections():
   ...
   reference_kind=re.search(r'ReferenceKind::([A-Za-z]+)',r['extra']).group(1)if r['kind']=='Reference'else None
   command=[str(binary),r['kind'],str(r['bits']),logical.hex(),str(offset),str(width),r['method'],reference_kind or 'None']
   observed=subprocess.check_output(command,text=True,timeout=5).strip();row=dict(name=r['name'],kind=r['kind'],method=r['method'],bits=r['bits'],data_hex=logical.hex(),source_data_hex=bytes(r['data']).hex(),declared=r['declared'],number=r['number'],reference_kind=reference_kind,field_offset=offset,field_bits=width,omega_error=r['error'],omega_expected=r['expected'],object_method=observed)
