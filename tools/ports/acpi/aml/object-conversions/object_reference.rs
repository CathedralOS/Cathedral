...
fn main(){
 ...
 let value=match a[1].as_str(){"Integer"=>Object::Integer(a[4].parse().unwrap()),"String"=>Object::String(String::from_utf8(data).unwrap()),"Buffer"=>Object::Buffer(data),"Field"=>Object::BufferField{buffer:Object::Buffer(data).wrap(),offset:a[4].parse().unwrap(),length:a[5].parse().unwrap()},"Reference"=>Object::Reference{kind:match a[7].as_str(){"Named"=>ReferenceKind::Named,"RefOf"=>ReferenceKind::RefOf,"Local"=>ReferenceKind::Local,"Arg"=>ReferenceKind::Arg,"Index"=>ReferenceKind::Index,"Unresolved"=>ReferenceKind::Unresolved,_=>panic!("unknown reference kind")},inner:Object::Integer(42).wrap()},"Package"=>Object::Package(vec![]),_=>Object::Uninitialized};
