...
def main():
 ...
 assert runtime['runner_sha256']==check.sha(Path('/tmp/cathedral-acpi-object-conversion-runner/release/cathedral-acpi-checked-runner'))
 ...
 for row in host['rows']:
  if row['kind']=='Reference':assert row['reference_kind']==row['name'].split('_')[1]
 history=HERE/'history/prepublication'
 old=json.loads((history/'tools/ports/acpi/aml/object-conversions/manifest.json').read_text())
 for path,digest in old['files_sha256'].items():assert check.sha(history/path)==digest,'historical artifact changed: '+path
 print('PASS',len(rows),'conversion preflight pairs,3constant pairs,',len(host['rows']),'Object observations plus66 explicit observations; current closure and historical artifacts verified')
