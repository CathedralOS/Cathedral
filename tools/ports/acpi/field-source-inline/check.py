...
def rows(group):
    return (fixtures if group == 'inline' else fixtures.previous).cases()


def module_source(group, selected):
    source, entries = (fixtures if group == 'inline' else fixtures.previous).render(selected)
    receiver = group.title()+'PayloadSuite'
...
def main():
    ...
    assert json.loads((HERE/'cases.json').read_text()) == rows('inline')
    assert json.loads((fixtures.BASE/'cases.json').read_text()) == rows('object')
    ...
    def run_batch(batch):
        ...
        with tempfile.TemporaryDirectory(prefix='cathedral-field-source-inline-') as directory:
            work=Path(directory)
