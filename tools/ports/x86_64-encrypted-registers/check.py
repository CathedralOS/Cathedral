...
def main():
 p=argparse.ArgumentParser();p.add_argument('--omega',type=Path,default=Path('/tmp/cathedral-omega-eaa7993/release/omega'));p.add_argument('--host-only',action='store_true');p.add_argument('--start',type=int,default=0);p.add_argument('--end',type=int,default=10);args=p.parse_args()
 ...
 print('PASS 100 pinned pure-expression observations across10 isolated profiles',flush=True)
 ...
 def evaluate(body,mutation=None):
  ...
  with tempfile.TemporaryDirectory(prefix='cathedral-encrypted-register-') as folder:
 ...
 if args.end==10:
  body=(HERE/'cases'/'batch-05.omg').read_text()
  evaluate(body,('observation.frame==expected','observation.frame!=expected'));print('PASS observed CR3 body mutation',flush=True)
  evaluate(body,('apic.frame==8192 && apic.raw_flags==140737488363521','apic.frame==4096 && apic.raw_flags==140737488363521'));print('PASS observed APIC mask body mutation',flush=True)
 print('PASS selected encrypted register cases',flush=True)
