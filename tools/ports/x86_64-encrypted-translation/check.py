...
def main():
 ...
 print('PASS 330 actual pinned mapper calls across11 isolated profiles',flush=True)
 ...
 def evaluate(body,mutation=None):
  ...
  with tempfile.TemporaryDirectory(prefix='cathedral-encrypted-translation-') as folder:
 ...
 if args.end==55:
  body=(HERE/'extras.omg').read_text();evaluate(body);print('PASS capture mismatch and boundary extras',flush=True)
  for old,new in [('expected_address==4096','expected_address==8192'),('expected_address==8192','expected_address==4096'),('expected_address==12288','expected_address==0')]:
   evaluate(body,(old,new));print('PASS capture body mutation',old,flush=True)
 print('PASS selected encrypted translation cases',flush=True)
