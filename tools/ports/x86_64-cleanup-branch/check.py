#!/usr/bin/env python3
"""Demand evaluated branch cleanup and recorded Rust deallocation order."""
...
def main():
 ...
 if not args.controls_only:
  for start in range(args.start,min(args.end or len(blocks),len(blocks)),4):
   selected=list(range(start,min(start+4,args.end or len(blocks),len(blocks))));evaluate(fixture(selected));print('PASS cleanup',selected,flush=True)
 if args.end is None:
  extra=(HERE/'extras.omg').read_text();prefix_extra=extra[:extra.index('machine invalid_page')]
  names=['invalid_page','mismatch','all_empty','selected_only','last_neighbor']
  ...
  for name,block in zip(names,extra_blocks):
   ...
   if name=='last_neighbor':evaluate(body,('found == true','found == false'));print('PASS full-array body mutation',flush=True)
  for n,old,new in [(48,'result.retire_count==3','result.retire_count==2'),(48,'result.frames_to_retire[0]==12288','result.frames_to_retire[0]==4096')]:
   evaluate(fixture([n]),(old,new));print('PASS body mutation',n,old,flush=True)
 print('PASS selected cleanup branches and controls',flush=True)
