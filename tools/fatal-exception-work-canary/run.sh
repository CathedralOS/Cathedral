#!/usr/bin/env bash
# Compile and inspect Cathedral's fixed-work fatal exception diagnostic without
# binding a physical entry, materializing a gate, or installing an IDT.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
CANARY_ROOT="$REPO_ROOT/tools/fatal-exception-work-canary"
OMEGA_REPO="${OMEGA_REPO:-$REPO_ROOT/../Omega}"
SCRATCH_DIR="$(mktemp -d "${TMPDIR:-/tmp}/cathedral-fatal-exception-work.XXXXXX")"
...
if ! command -v jq >/dev/null 2>&1; then
  echo "error: jq is required to validate fatal-exception work artifacts" >&2
  exit 2
fi
...
ASSERTIONS="$CANARY_ROOT/assert-work.jq"
...
echo "Cathedral fatal-exception fixed-work canary passed"
