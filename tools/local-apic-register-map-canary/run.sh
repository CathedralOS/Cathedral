#!/usr/bin/env bash
# Compile Cathedral's pure local-APIC register correspondence without
# accessing MMIO, MSRs, or interrupt control.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
CANARY_ROOT="$REPO_ROOT/tools/local-apic-register-map-canary"
OMEGA_REPO="${OMEGA_REPO:-$REPO_ROOT/../Omega}"
SCRATCH_DIR="$(mktemp -d "${TMPDIR:-/tmp}/cathedral-local-apic-map.XXXXXX")"
...
if ! command -v jq >/dev/null 2>&1; then
  echo "error: jq is required to validate the typed local-APIC register map" >&2
  exit 2
fi
...
MAP_ASSERTIONS="$CANARY_ROOT/assert-register-map.jq"

for artifact in "$TYPED" "$CONTRACTS" "$MAP_ASSERTIONS"; do
  [[ -f "$artifact" ]] || {
...
jq -s -e -f "$MAP_ASSERTIONS" "$TYPED" "$CONTRACTS" >/dev/null

echo "Cathedral local-APIC register-map canary passed"
