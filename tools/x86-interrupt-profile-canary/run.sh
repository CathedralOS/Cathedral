#!/usr/bin/env bash
# Compile Cathedral's pure x86 vector/IST composition without executing or
# granting any privileged operation.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
CANARY_ROOT="$REPO_ROOT/tools/x86-interrupt-profile-canary"
OMEGA_REPO="${OMEGA_REPO:-$REPO_ROOT/../Omega}"
SCRATCH_DIR="$(mktemp -d "${TMPDIR:-/tmp}/cathedral-x86-profile.XXXXXX")"
...
if ! command -v jq >/dev/null 2>&1; then
  echo "error: jq is required to validate the typed x86 interrupt profile" >&2
  exit 2
fi
...
PROFILE_ASSERTIONS="$CANARY_ROOT/assert-profile.jq"

for artifact in "$TYPED" "$CONTRACTS" "$PROFILE_ASSERTIONS"; do
  [[ -f "$artifact" ]] || {
...
jq -s -e -f "$PROFILE_ASSERTIONS" "$TYPED" "$CONTRACTS" >/dev/null

echo "Cathedral x86 interrupt-profile canary passed"
