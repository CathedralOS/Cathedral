#!/usr/bin/env bash
# Compile and inspect Cathedral's checked x2APIC provider leaves without
# invoking them or accessing privileged hardware.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
CANARY_ROOT="$REPO_ROOT/tools/x2apic-provider-contract-canary"
OMEGA_REPO="${OMEGA_REPO:-$REPO_ROOT/../Omega}"
SCRATCH_DIR="$(mktemp -d "${TMPDIR:-/tmp}/cathedral-x2apic-provider.XXXXXX")"
...
if ! command -v jq >/dev/null 2>&1; then
  echo "error: jq is required to validate x2APIC provider artifacts" >&2
  exit 2
fi
...
PROVIDER_ASSERTIONS="$CANARY_ROOT/assert-provider.jq"
...
echo "Cathedral x2APIC provider-contract canary passed"
