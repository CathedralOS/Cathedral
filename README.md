...
- **Instant file copying.** Copying a file, no matter how large, is instantaenous. The folder hierarchy is decoupled from the underlying file storage, so you only pay a cost when the file is changed (copy-on-write), or when the file is copied to another storage device.
...
- **Zero-cost sandboxing.** Docker and virtual machines are obsoleted by design. It is trivial to spin up a zero-cost nested Cathedral instance, referred to a Matrix. These cannot leak into the owning operating system. Even the main desktop is a Matrix. Detecting whether we are "in a Matrix" is almost impossible for malware, because everything is. We control what goes into and leaves Matrices. This works because the filesystem can share all system files as read-only.
...
- **AI agents you can hand real power.** An agent holds the *right to use* a secret, never the secret's bytes. A prompt-injected agent can *cannot* exfiltrate keys, because the bytes never enter its reach. The worst a compromised agent can do is limited to "use this key, read-only, rate-limited, within this sandbox."
